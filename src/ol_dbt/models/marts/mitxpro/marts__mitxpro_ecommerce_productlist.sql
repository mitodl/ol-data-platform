...
with ecommerce_productversion as (
    select *
...
with ecommerce_productversion_latest as (
    select *
    from (
            select * ,
...
,'<a href="https://xpro.mit.edu/checkout?product=' || cast(ecommerce_product.product_id as varchar(50)) || '">' || course_runs.courserun_readable_id  || '</a>' as link
...
where ecommerce_product.product_type = 'program'  and program_runs.programrun_id is not null
