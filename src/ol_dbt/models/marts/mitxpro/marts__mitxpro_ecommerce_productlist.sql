with
    ecommerce_product as (select * from {{ ref("int__mitxpro__ecommerce_product") }}),
    ecommerce_productversion as (select * from {{ ref("int__mitxpro__ecommerce_productversion") }}),
    course_runs as (select * from {{ ref("int__mitxpro__course_runs") }}),
    programs as (select * from {{ ref("int__mitxpro__programs") }}),
    program_runs as (select * from {{ ref("int__mitxpro__program_runs") }}),
    courses as (select * from {{ ref("int__mitxpro__courses") }}),
    course_to_topics as (select * from {{ ref("int__mitxpro__courses_to_topics") }}),
    coursetopic as (select * from {{ ref("int__mitxpro__coursetopic") }}),
    ...
    ecommerce_course_to_topics as (
        select course_to_topics.course_id, array_join(array_agg(coursetopic.coursetopic_name), ', ') as coursetopic_name
        from course_to_topics
        inner join coursetopic on course_to_topics.coursetopic_id = coursetopic.coursetopic_id
...
left join ecommerce_productversion_latest on ecommerce_product.product_id = ecommerce_productversion_latest.product_id
inner join course_runs on ecommerce_product.courserun_id = course_runs.courserun_id
inner join courses on course_runs.course_id = courses.course_id
left join programs on courses.program_id = programs.program_id
left join ecommerce_course_to_topics on course_runs.course_id = ecommerce_course_to_topics.course_id
...
from ecommerce_product
left join ecommerce_productversion_latest on ecommerce_product.product_id = ecommerce_productversion_latest.product_id
inner join programs on ecommerce_product.program_id = programs.program_id
inner join program_runs on programs.program_id = program_runs.program_id
