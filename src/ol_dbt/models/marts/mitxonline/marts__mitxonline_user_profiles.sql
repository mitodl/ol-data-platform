with
    ...
    income as (select * from {{ ref("int__mitxonline__flexiblepricing_flexiblepriceapplication") }}),
    user_income as (
        select
            user_id,
...
from users
left join user_income on users.user_id = user_income.user_id and user_income.rnk = 1
