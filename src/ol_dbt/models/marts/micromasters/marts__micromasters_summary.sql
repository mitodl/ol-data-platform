with
    micromasters_enrollments as (select * from {{ ref("int__micromasters__course_enrollments") }}),
    micromasters_course_certificates as (select * from {{ ref("int__micromasters__course_certificates") }}),
    micromasters_program_certificates as (select * from {{ ref("int__micromasters__program_certificates") }}),
    mitx_programs as (select * from {{ ref("int__mitx__programs") }}),
    enrollments as (
        select
            case
                when mitx_programs.is_dedp_program = true
                then 'Data, Economics, and Design of Policy'
                else micromasters_enrollments.program_title
            end as program_title,
            count(
                distinct micromasters_enrollments.courserun_readable_id || micromasters_enrollments.user_email
        ...
        inner join
            mitx_programs
            on micromasters_enrollments.micromasters_program_id = mitx_programs.micromasters_program_id
    ...
    course_certs as (
        select
            case
                when mitx_programs.is_dedp_program = true
                then 'Data, Economics, and Design of Policy'
                else micromasters_course_certificates.program_title
            end as program_title,
            count(
                distinct
                micromasters_course_certificates.courserun_readable_id || micromasters_course_certificates.user_email
        ...
        inner join
            mitx_programs
            on micromasters_course_certificates.micromasters_program_id = mitx_programs.micromasters_program_id
    ...
    program_certs as (
        select
            case
                when mitx_programs.is_dedp_program = true
                then 'Data, Economics, and Design of Policy'
                else micromasters_program_certificates.program_title
            end as program_title,
...
from enrollments
left join course_certs on enrollments.program_title = course_certs.program_title
left join program_certs on enrollments.program_title = program_certs.program_title
