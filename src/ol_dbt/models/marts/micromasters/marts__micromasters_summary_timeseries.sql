with
    enrollments as (select * from {{ ref("int__micromasters__course_enrollments") }}),
    course_certificates as (select * from {{ ref("int__micromasters__course_certificates") }}),
    program_certificates as (select * from {{ ref("int__micromasters__program_certificates") }}),
    timeseries as (
        select end_date
        from unnest(sequence(date '2014-01-01', current_date, interval '1' month) || current_date) as t(end_date)  -- noqa
    ),
    enrollments_by_program as (
        ...
        from enrollments
        inner join
            timeseries on from_iso8601_timestamp(enrollments.courserunenrollment_created_on) < timeseries.end_date
    ...
    course_certificates_by_program as (
        ...
        group by
            course_certificates.micromasters_program_id, course_certificates.mitxonline_program_id, timeseries.end_date
    ),
    ...
    program_certificates_combined as (
        select end_date, micromasters_program_id, mitxonline_program_id, program_certificates
