with
    mitxonline_product as (select * from {{ ref("int__mitxonline__ecommerce_product") }}),
    mitxonline_course_runs as (select * from {{ ref("int__mitxonline__course_runs") }}),
    mitxonline_courses as (select * from {{ ref("int__mitxonline__courses") }}),
    mitxonline_programs as (select * from {{ ref("int__mitxonline__programs") }}),
    mitxpro_product as (select * from {{ ref("int__mitxpro__ecommerce_product") }}),
    mitxpro_course_runs as (select * from {{ ref("int__mitxpro__course_runs") }}),
    mitxpro_courses as (select * from {{ ref("int__mitxpro__courses") }}),
    mitxpro_programs as (select * from {{ ref("int__mitxpro__programs") }}),
    mitxpro_program_runs as (select * from {{ ref("int__mitxpro__program_runs") }}),
    edxorg_product as (select * from {{ ref("int__edxorg__mitx_product") }}),
    edxorg_runs as (select * from {{ ref("int__edxorg__mitx_courseruns") }}),
    mitxonline_product_view as (
        select
            ...
            if(mitxonline_course_runs.courserun_is_self_paced = true, 'Self-paced', 'Instructor-paced') as pace,
            if(
                mitxonline_courses.course_page_is_live = true and mitxonline_courses.course_is_live = true, true, false
            ) as is_live
        from mitxonline_course_runs
        inner join mitxonline_courses on mitxonline_course_runs.course_id = mitxonline_courses.course_id
        left join mitxonline_product on mitxonline_course_runs.courserun_id = mitxonline_product.courserun_id
        ...
        left join mitxonline_product on mitxonline_programs.program_readable_id = mitxonline_product.program_readable_id
    ...
    mitxpro_product_view as (
        select
            ...
            coalesce(
                mitxpro_courses.cms_certificate_ceus, mitxpro_programs.cms_certificate_ceus
        ...
        left join mitxpro_course_runs on mitxpro_product.courserun_id = mitxpro_course_runs.courserun_id
        left join mitxpro_courses on mitxpro_course_runs.course_id = mitxpro_courses.course_id
        left join mitxpro_program_runs on mitxpro_product.program_id = mitxpro_program_runs.program_id
        left join mitxpro_programs on mitxpro_program_runs.program_id = mitxpro_programs.program_id
    ...
    edxorg_product_view as (
        select
            ...
            if(
                edxorg_runs.micromasters_program_id is not null, 'MicroMasters Credential', 'Certificate of Completion'
            ) as certification_type
        from edxorg_product
        inner join edxorg_runs on edxorg_product.courserun_readable_id = edxorg_runs.courserun_readable_id
        left join
            mitxonline_product_view
            on edxorg_product.courserun_readable_id = mitxonline_product_view.product_readable_id
        where edxorg_product.courserun_mode = 'verified' and mitxonline_product_view.product_readable_id is null
