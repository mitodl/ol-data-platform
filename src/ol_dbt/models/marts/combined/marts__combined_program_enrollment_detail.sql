with
    mitxpro__programenrollments as (select * from {{ ref("int__mitxpro__programenrollments") }}),
    mitx_programs as (select * from {{ ref("int__mitx__program_requirements") }}),
    mitxpro__programs as (select * from {{ ref("int__mitxpro__programs") }}),
    mitxpro__program_certificates as (select * from {{ ref("int__mitxpro__program_certificates") }}),
    mitxpro__users as (select * from {{ ref("int__mitxpro__users") }}),
    mitxonline__programenrollments as (select * from {{ ref("int__mitxonline__programenrollments") }}),
    mitxonline__programs as (select * from {{ ref("int__mitxonline__programs") }}),
    mitxonline__users as (select * from {{ ref("int__mitxonline__users") }}),
    mitx__programs as (select * from {{ ref("int__mitx__programs") }}),
    mitxonline__program_certificates as (select * from {{ ref("int__mitxonline__program_certificates") }}),
    edx_program_certificates as (select * from {{ ref("int__edxorg__mitx_program_certificates") }}),
    edx_program_enrollments as (select * from {{ ref("int__edxorg__mitx_program_enrollments") }}),
    edxorg__users as (select * from {{ ref("int__edxorg__mitx_users") }}),
    micromasters__program_enrollments as (select * from {{ ref("int__micromasters__program_enrollments") }}),
    edxorg_mitx_program_courses as (select * from {{ ref("int__edxorg__mitx_program_courses") }}),
    mitx__courses as (select * from {{ ref("int__mitx__courses") }}),
    mitxpro__courses as (select * from {{ ref("int__mitxpro__courses") }}),
    combined_enrollments as (select * from {{ ref("int__combined__courserun_enrollments") }}),
    combined_courseruns as (select * from {{ ref("int__combined__course_runs") }}),
    combined_programs as (
        select
            ...
            if(
                mitxpro__program_certificates.programcertificate_url is not null, true, false
            ) as user_has_completed_program
        from mitxpro__programenrollments
        inner join mitxpro__programs on mitxpro__programenrollments.program_id = mitxpro__programs.program_id
        left join mitxpro__users on mitxpro__programenrollments.user_id = mitxpro__users.user_id
        left join
            mitxpro__program_certificates
            on mitxpro__programenrollments.program_id = mitxpro__program_certificates.program_id
        ...
        select
            ...
            if(
                mitxonline__program_certificates.programcertificate_url is not null, true, false
            ) as user_has_completed_program
        from mitxonline__programenrollments
        inner join mitxonline__programs on mitxonline__programenrollments.program_id = mitxonline__programs.program_id
        left join mitxonline__users on mitxonline__programenrollments.user_id = mitxonline__users.user_id
        left join
            mitxonline__program_certificates
            on mitxonline__programenrollments.user_id = mitxonline__program_certificates.user_id
        ...
        select
            ...
            {{
                generate_micromasters_program_readable_id(
                    "edx_program_enrollments.micromasters_program_id", "edx_program_enrollments.program_title"
                )
        ...
        left join edxorg__users on edx_program_enrollments.user_id = edxorg__users.user_id
        left join
            edx_program_certificates
            on edx_program_enrollments.program_uuid = edx_program_certificates.program_uuid
        ...
        left join mitxonline__programs on mitx__programs.mitxonline_program_id = mitxonline__programs.program_id
        left join
            edx_program_certificates
            on micromasters__program_enrollments.micromasters_program_id
            = edx_program_certificates.micromasters_program_id
            and micromasters__program_enrollments.user_edxorg_id = edx_program_certificates.user_id
        left join
            edx_program_enrollments
            on micromasters__program_enrollments.micromasters_program_id
    ...
    courses_in_programs as (
        select mitx__courses.course_readable_id, mitxonline__programs.program_readable_id
        from mitx_programs
        inner join mitx__courses on mitx_programs.course_number = mitx__courses.course_number
        left join mitxonline__programs on mitx_programs.mitxonline_program_id = mitxonline__programs.program_id
        where mitx__courses.is_on_mitxonline = true

        union all

        select mitx__courses.course_readable_id, null as program_readable_id
        from mitx_programs
        inner join mitx__courses on mitx_programs.course_number = mitx__courses.course_number
        ...
        select
            coalesce(
                mitx__courses.course_readable_id, edxorg_mitx_program_courses.course_readable_id
            ) as course_readable_id,
            null as program_readable_id
        from edxorg_mitx_program_courses
        left join mitx_programs on edxorg_mitx_program_courses.program_name = mitx_programs.program_title
        left join
            mitx__courses
            on replace(edxorg_mitx_program_courses.course_readable_id, 'MITx/', '')
            = replace(mitx__courses.course_readable_id, 'course-v1:MITxT+', '')
        where mitx_programs.program_title is null

        union all

        select mitxpro__courses.course_readable_id, mitxpro__programs.program_readable_id
        from mitxpro__courses
        inner join mitxpro__programs on mitxpro__courses.program_id = mitxpro__programs.program_id
    ),
    final_combined_programs as (
        select
            ...
            cast(
                substring(combined_programs.programcertificate_created_on, 1, 10) as date
            ) as programcertificate_created_on_date,
            sum(case when upper(combined_enrollments.course_title) like '%CAPSTONE%' then 1 else 0 end) as capstone_sum,
            min(cast(substring(combined_courseruns.courserun_start_on, 1, 10) as date)) as first_courserun_start_on_date
            ,
            count(distinct combined_enrollments.course_readable_id) as unique_courses_taken_in_program
        from combined_programs
        left join courses_in_programs on combined_programs.program_readable_id = courses_in_programs.program_readable_id
        left join
            combined_enrollments
            on cast(combined_programs.user_id as varchar) = combined_enrollments.user_id
...
select
    ...
    case when final_combined_programs.capstone_sum > 0 then 'Y' else 'N' end as capstone_indicator,
    date_diff(
        'day',
