...
with
    bootcamps__ecommerce_order as (select * from {{ ref("int__bootcamps__ecommerce_order") }}),
    bootcamps__receipt as (select * from {{ ref("int__bootcamps__ecommerce_receipt") }}),
    mitxpro__ecommerce_allorders as (select * from {{ ref("int__mitxpro__ecommerce_allorders") }}),
    mitxonline__ecommerce_order as (select * from {{ ref("int__mitxonline__ecommerce_order") }}),
    mitxonline__transaction as (select * from {{ ref("int__mitxonline__ecommerce_transaction") }}),
    mitxpro__lines as (select * from {{ ref("int__mitxpro__ecommerce_line") }}),
    mitxpro__programruns as (select * from {{ ref("int__mitxpro__program_runs") }}),
    mitxpro__receipts as (
        select * from {{ ref("int__mitxpro__ecommerce_receipt") }} where receipt_transaction_status != 'ERROR'
    ),
    mitxpro__ecommerce_order as (select * from {{ ref("int__mitxpro__ecommerce_order") }}),
    mitxpro__ecommerce_allcoupons as (select * from {{ ref("int__mitxpro__ecommerce_allcoupons") }}),
    micromasters_orders as (select * from {{ ref("int__micromasters__orders") }}),
    mitxonline_orders as (
        ...
        left join
            mitxonline__transaction
            on mitxonline__ecommerce_order.transaction_id = mitxonline__transaction.transaction_id
        left join
            mitxonline__transaction as mitxonline__refund
            on mitxonline__ecommerce_order.order_id = mitxonline__refund.order_id
    ...
    bootcamps_orders as (
        ...
        left join bootcamps__receipt on bootcamps__ecommerce_order.order_id = bootcamps__receipt.order_id
    ),
    mitxpro_orders as (
        select
            ...
            concat(
                'xpro-b2c-production-', cast(mitxpro__ecommerce_allorders.order_id as varchar)
        ...
        left join mitxpro__ecommerce_order on mitxpro__ecommerce_allorders.order_id = mitxpro__ecommerce_order.order_id
        left join mitxpro__receipts on mitxpro__ecommerce_order.order_id = mitxpro__receipts.order_id
        left join mitxpro__lines on mitxpro__ecommerce_allorders.line_id = mitxpro__lines.line_id
        left join
            mitxpro__ecommerce_allcoupons
            on mitxpro__ecommerce_allorders.coupon_id = mitxpro__ecommerce_allcoupons.coupon_id
        left join mitxpro__programruns on mitxpro__lines.programrun_id = mitxpro__programruns.programrun_id
    ...
    combined_orders as (
        select
            ...
            case
                ...
                then cast(product_price - cast(substring(discount_amount_text, 14) as decimal(38, 2)) as varchar)
        ...
        select
            ...
            case
                ...
                then cast(line_price - cast(substring(coupon_discount_amount_text, 14) as decimal(38, 2)) as varchar)
