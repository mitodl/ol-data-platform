with
    mitxonline_discount as (select * from {{ ref("int__mitxonline__ecommerce_discount") }}),
    mitxonline_discountproduct as (select * from {{ ref("int__mitxonline__ecommerce_discountproduct") }}),
    mitxonline_product as (select * from {{ ref("int__mitxonline__ecommerce_product") }}),
    mitxonline_order as (select * from {{ ref("int__mitxonline__ecommerce_order") }}),
    mitxpro_all_coupons as (select * from {{ ref("marts__mitxpro_all_coupons") }}),
    ...
    mitxonline_product_joins as (
        select
            ...
            coalesce(
                mitxonline_product.courserun_readable_id, mitxonline_product.program_readable_id
            ) as product_readable_id
        from mitxonline_discountproduct
        left join mitxonline_product on mitxonline_discountproduct.product_id = mitxonline_product.product_id
        group by
            mitxonline_discountproduct.discount_id,
            coalesce(mitxonline_product.courserun_readable_id, mitxonline_product.program_readable_id)
    )
...
left join mitxonline_product_joins on mitxonline_discount.discount_id = mitxonline_product_joins.discount_id
left join mitxonline_order_sum on mitxonline_discount.discount_code = mitxonline_order_sum.discount_code
