with
    combined_engagements as (
        select
            ...
            count(
                distinct case when coursestructure_block_category = 'discussion' then coursestructure_block_id end
            ) as total_courserun_discussions,
            count(
                distinct case when coursestructure_block_category = 'video' then coursestructure_block_id end
            ) as total_courserun_videos,
            count(
                distinct case when coursestructure_block_category = 'problem' then coursestructure_block_id end
            ) as total_courserun_problems
        from {{ ref("int__combined__course_structure") }}
        group by platform, courserun_readable_id
    ),
    combined_runs as (select * from {{ ref("int__combined__course_runs") }} where courserun_readable_id is not null),
    combined_enrollments as (
        select platform, courserun_readable_id, user_username
        from {{ ref("int__combined__courserun_enrollments") }}
        where courserun_readable_id is not null
        group by platform, courserun_readable_id, user_username
    ),
    combined_user_video as (
        ...
        group by '{{ var("mitxonline") }}', user_username, courserun_readable_id
        ...
        where useractivity_event_type = 'play_video'
        group by '{{ var("edxorg") }}', user_username, courserun_readable_id
        ...
        where useractivity_event_type = 'play_video'
        group by '{{ var("mitxpro") }}', user_username, courserun_readable_id
        ...
        where useractivity_event_type = 'play_video'
        group by '{{ var("residential") }}', user_username, courserun_readable_id
    ),
    ...
    user_video_correction as (
        select platform, courserun_readable_id, max(videos_user_watched) as max_videos_per_courserun
        from combined_user_video
        group by platform, courserun_readable_id
    ),
    user_problem_correction as (
        select platform, courserun_readable_id, max(problems_user_submitted) as max_problems_per_courserun
        from combined_user_problem
        group by platform, courserun_readable_id
    ),
    user_discussion_correction as (
        select platform, courserun_readable_id, max(user_discussion_count) as max_discussions_per_courserun
...
select
    ...
    case
        when
            combined_engagements.total_courserun_discussions >= user_discussion_correction.max_discussions_per_courserun
...
inner join
    ...
    on combined_runs.courserun_readable_id = combined_engagements.courserun_readable_id
    and combined_runs.platform = combined_engagements.platform
inner join
    combined_enrollments
    on combined_runs.courserun_readable_id = combined_enrollments.courserun_readable_id
    and combined_runs.platform = combined_enrollments.platform
left join
    combined_user_video
    on combined_enrollments.courserun_readable_id = combined_user_video.courserun_readable_id
...
left join
    combined_user_problem
    on combined_enrollments.courserun_readable_id = combined_user_problem.courserun_readable_id
...
left join
    combined_user_discussion
    on combined_enrollments.courserun_readable_id = combined_user_discussion.courserun_readable_id
...
left join
    user_video_correction
    on combined_runs.courserun_readable_id = user_video_correction.courserun_readable_id
...
left join
    user_discussion_correction
    on combined_runs.courserun_readable_id = user_discussion_correction.courserun_readable_id
