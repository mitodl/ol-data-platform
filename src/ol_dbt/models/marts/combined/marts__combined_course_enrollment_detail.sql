with
    ...
    mitxpro_enrollments as (select * from {{ ref("int__mitxpro__courserunenrollments") }}),
    combined_courseruns as (select * from {{ ref("int__combined__course_runs") }}),
    combined_users as (select * from {{ ref("int__combined__users") }}),
    mitxonline_transactions as (
        select order_id, max(transaction_timestamp) as payment_timestamp
        from {{ ref("int__mitxonline__ecommerce_transaction") }}
        where transaction_type = 'payment'
        group by order_id
    ),
    mitxonline_completed_orders as (
        select *, row_number() over (partition by user_id, courserun_id order by order_created_on desc) as row_num
    ...
    mitxpro_completed_orders as (
        select * from {{ ref("int__mitxpro__ecommerce_order") }} where order_state in ('fulfilled', 'refunded')
    ),
    mitxpro_receipts as (
        select order_id, max(receipt_payment_timestamp) as payment_timestamp
        from {{ ref("int__mitxpro__ecommerce_receipt") }}
        where receipt_transaction_status != 'ERROR'
        group by order_id
    ),
    bootcamps_completed_orders as (
        select * from {{ ref("int__bootcamps__ecommerce_order") }} where order_state in ('fulfilled', 'refunded')
    ),
    bootcamps_receipts as (
        select order_id, max(receipt_payment_timestamp) as receipt_payment_timestamp
        from {{ ref("int__bootcamps__ecommerce_receipt") }}
        group by order_id
    ),
    mitxpro__ecommerce_line as (select * from {{ ref("int__mitxpro__ecommerce_line") }}),
    mitxonline_certificates as (select * from {{ ref("int__mitxonline__courserun_certificates") }}),
    combined_enrollment_detail as (
        select
            ...
            if(
                mitxonline_completed_orders.order_id is not null,
                coalesce(mitxonline_transactions.payment_timestamp, mitxonline_completed_orders.order_created_on),
                null
            ) as courserunenrollment_upgraded_on
        from combined_enrollments
        left join
            combined_users
            on combined_enrollments.user_id = combined_users.user_id
            and combined_enrollments.platform = combined_users.platform
        left join
            mitxonline_completed_orders
            on combined_enrollments.user_id = cast(mitxonline_completed_orders.user_id as varchar)
        ...
        left join mitxonline_transactions on mitxonline_completed_orders.order_id = mitxonline_transactions.order_id
        ...
        select
            ...
            combined_courseruns.courserun_end_on,
            if(
                combined_courseruns.courserun_is_current is null, false, combined_courseruns.courserun_is_current
            ...
            combined_users.user_gender,
            case
                when mitxonline_certificates.courseruncertificate_is_revoked = false
                then true
                when combined_enrollments.courseruncertificate_created_on is not null
                then true
                else false
            ...
            coalesce(
                mitxonline_certificates.courseruncertificate_url, combined_enrollments.courseruncertificate_url
            ) as courseruncertificate_url,
            coalesce(
                mitxonline_certificates.courseruncertificate_uuid, combined_enrollments.courseruncertificate_uuid
            ...
            combined_enrollments.courserun_upgrade_deadline,
            if(
                micromasters_completed_orders.order_id is not null,
                coalesce(
        ...
        left join
            combined_users
            on combined_enrollments.user_username = combined_users.user_username
            and combined_enrollments.platform = combined_users.platform
        left join micromasters_users on combined_enrollments.user_username = micromasters_users.user_edxorg_username
        left join
            micromasters_completed_orders
            on micromasters_users.user_id = micromasters_completed_orders.user_id
        ...
        select
            ...
            combined_enrollments.courserun_upgrade_deadline,
            if(
                mitxpro_completed_orders.order_id is not null,
        ...
        left join
            combined_users
            on mitxpro_enrollments.user_username = combined_users.user_username
            and combined_enrollments.platform = combined_users.platform
        left join mitxpro_completed_orders on mitxpro_enrollments.ecommerce_order_id = mitxpro_completed_orders.order_id
        left join
            combined_courseruns on mitxpro_enrollments.courserun_readable_id = combined_courseruns.courserun_readable_id
        left join mitxpro__ecommerce_line on mitxpro_completed_orders.order_id = mitxpro__ecommerce_line.order_id
        left join mitxpro_receipts on mitxpro_completed_orders.order_id = mitxpro_receipts.order_id
        ...
        select
            ...
            combined_enrollments.courserun_upgrade_deadline,
            if(
                bootcamps_completed_orders.order_id is not null,
        ...
        left join
            ...
            and combined_enrollments.platform = combined_users.platform
        left join
            bootcamps_completed_orders
            on combined_enrollments.user_username = bootcamps_completed_orders.user_username
            and combined_enrollments.courserun_id = bootcamps_completed_orders.courserun_id
        left join
            combined_courseruns
            on combined_enrollments.courserun_readable_id = combined_courseruns.courserun_readable_id
            and combined_enrollments.platform = combined_courseruns.platform
        left join bootcamps_receipts on bootcamps_completed_orders.order_id = bootcamps_receipts.order_id
...
select
    ...
    {{
        generate_hash_id(
            "cast(order_id as varchar)         || cast(coalesce(line_id, 9) as varchar)         || platform"
        )
    }} as combined_orders_hash_id,
    course_readable_id,
    course_title,
    courserun_id,
    ...
    case
        ...
        else {{ generate_hash_id("user_full_name || platform") }}
