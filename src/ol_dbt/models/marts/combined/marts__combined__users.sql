...
with
    mitx__users as (select * from {{ ref("int__mitx__users") }}),
    non_mitx_users as (select * from {{ ref("int__combined__users") }}),
    combined_enrollments as (select * from {{ ref("int__combined__courserun_enrollments") }}),
    combined_programs as (select * from {{ ref("marts__combined_program_enrollment_detail") }}),
    orders as (select * from {{ ref("marts__combined__orders") }}),
    income as (select * from {{ ref("marts__mitxonline_user_profiles") }}),
    ...
    program_stats as (
        select
            ...
            ) as dedp_program_cred_count
        from combined_programs
        group by user_email
    ),
    orders_stats as (
        select user_email, sum(order_total_price_paid) as total_amount_paid_orders from orders group by user_email
    ),
    course_stats as (
        select
            ...
            count(
                distinct
                case
                    when combined_enrollments.courserungrade_is_passing = true then combined_enrollments.course_title
                end
    ...
    combined_users as (
        select
            ...
            case
                when user_is_active_on_mitxonline then user_is_active_on_mitxonline else user_is_active_on_edxorg
