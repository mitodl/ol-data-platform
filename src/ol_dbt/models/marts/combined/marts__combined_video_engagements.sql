with
    ...
    combined_video_structure as (select * from {{ ref("int__combined__course_videos") }}),
...
select
    ...
    coalesce(
        combined_video_structure.video_duration, combined_video_engagements.useractivity_video_duration
    ) as video_duration
from combined_video_engagements
inner join
    combined_runs
    on combined_video_engagements.courserun_readable_id = combined_runs.courserun_readable_id
    and combined_video_engagements.platform = combined_runs.platform
left join
    combined_video_structure
    on combined_video_engagements.courserun_readable_id = combined_video_structure.courserun_readable_id
...
left join
    combined_users
    on combined_video_engagements.user_username = combined_users.user_username
