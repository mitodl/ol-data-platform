...
with
        ...
        from
            (
                select
                    *,
                    row_number() over (
                        partition by user_username order by openedx_user_id asc nulls last, user_joined_on desc
                    ) as row_num
                from users
            )
        where row_num = 1
    ...
    mitxonline_edxorg_users as (
        select
            ...
            coalesce(
                mitxonline_users_view.user_address_country, edxorg_users_view.user_address_country
            ) as user_address_country,
            coalesce(
                mitxonline_users_view.user_address_state, edxorg_users_view.user_address_state_or_territory
            ) as user_address_state,
            coalesce(
                mitxonline_users_view.user_highest_education, edxorg_users_view.user_highest_education
            ) as user_highest_education,
        ...
        full outer join
            edxorg_users_view on mitxonline_users_view.user_edxorg_username = edxorg_users_view.user_edxorg_username
    )
...
left join mitxonline_edxorg_users on micromasters_users.user_id = mitxonline_edxorg_users.user_micromasters_id
