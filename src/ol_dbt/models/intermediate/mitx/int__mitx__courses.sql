with
    mitxonline_courses as (
        select course_id as mitxonline_course_id, course_title, course_is_live, course_readable_id, course_number
    ...
    edx_courseruns as (
        select
            *,
            row_number() over (
                partition by trim(course_number) order by courserun_start_date desc
    ...
    mitx_courses as (
        ...
        full join edx_courses on mitxonline_courses.course_number = edx_courses.course_number
    )
