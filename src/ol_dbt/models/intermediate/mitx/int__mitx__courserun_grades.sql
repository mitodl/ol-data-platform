...
with
    ...
    edxorg_non_program_course_grades as (
        select * from {{ ref("int__edxorg__mitx_courserun_grades") }} where micromasters_program_id is null
    ),
    all_program_course_grades as (select * from {{ ref("int__micromasters__course_grades") }}),
    mitxonline_non_dedp_course_grades as (
        ...
        left join
            all_program_course_grades
            on mitxonline_grades.courserun_readable_id = all_program_course_grades.courserun_readable_id
