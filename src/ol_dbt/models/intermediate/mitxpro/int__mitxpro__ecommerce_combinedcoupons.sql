...
, payment_transactions as (
    ...
    group by couponpayment_name) as couponpaymentversion
    inner join int__mitxpro__ecommerce_couponpaymentversion as ecommerce_couponpaymentversion
        on couponpaymentversion.couponpaymentversion_id = ecommerce_couponpaymentversion.couponpaymentversion_id
    inner join int__mitxpro__ecommerce_couponversion as couponversion
...
select
    ...
    , array_join(array_distinct(array_agg(ecommerce_order.order_state)), ', ') as combined_order_status
from int__mitxpro__ecommerce_coupon as coupon
left join int__mitxpro__ecommerce_productcouponassignment as productcouponassignment
    on productcouponassignment.coupon_id = coupon.coupon_id
left join int__mitxpro__ecommerce_couponversion as ecommerce_couponversion
    on coupon.coupon_id = ecommerce_couponversion.couponversion_id
left join int__mitxpro__ecommerce_couponpaymentversion as ecommerce_couponpaymentversion
    on ecommerce_couponpaymentversion.couponpaymentversion_id = ecommerce_couponversion.couponpaymentversion_id
left join int__mitxpro__b2becommerce_b2border as b2b_ecommerce_b2border
    on ecommerce_couponpaymentversion.couponpaymentversion_id = b2b_ecommerce_b2border.couponpaymentversion_id
left join int__mitxpro__ecommerce_couponredemption as couponredemption
    on ecommerce_couponversion.couponversion_id = couponredemption.couponversion_id
left join int__mitxpro__ecommerce_order as ecommerce_order
    on ecommerce_order.order_id = couponredemption.order_id
left join int__mitxpro__users as users_user
...
left join int__mitxpro__ecommerce_line as ecommerce_line
    on ecommerce_line.order_id = ecommerce_order.order_id
left join int__mitxpro__program_runs as program_runs
    on ecommerce_line.programrun_id = program_runs.programrun_id
left join int__mitxpro__course_runs as course_runs
    on ecommerce_line.courserun_id = course_runs.courserun_id
left join int__mitxpro__courses as courses
    on course_runs.course_id = courses.course_id
left join int__mitxpro__ecommerce_productversion as productversion
...
select
    ...
    , ecommerce_coupon.coupon_code
    , ecommerce_coupon.coupon_created_on
    ...
    , replace(cast(json_extract(b2breceipt_data, '$.req_reference_number') as VARCHAR), '"', '') as req_reference_number
    , programrun_readable_id
    , program_title
    , courserun_readable_id
    , course_title
    ...
    , array_join(array_distinct(array_agg(ecommerce_order.order_id)), ', ') as order_ids
    , array_join(array_distinct(array_agg(ecommerce_order.order_state)), ', ') as combined_order_status
from int__mitxpro__b2becommerce_b2border as b2b_ecommerce_b2border
inner join int__mitxpro__ecommerce_couponpaymentversion as ecommerce_couponpaymentversion
    on ecommerce_couponpaymentversion.couponpaymentversion_id = b2b_ecommerce_b2border.couponpaymentversion_id
inner join int__mitxpro__ecommerce_coupon as ecommerce_coupon
    on ecommerce_coupon.coupon_id = b2b_ecommerce_b2border.b2bcoupon_id
inner join int__mitxpro__ecommerce_couponversion as ecommerce_couponversion
    on ecommerce_couponversion.coupon_id = ecommerce_coupon.coupon_id
left join int__mitxpro__ecommerce_couponredemption as ecommerce_couponredemption
...
left join int__mitxpro__users as users_user
    on ecommerce_order.order_purchaser_user_id = users_user.user_id
left join int__mitxpro__b2becommerce_b2breceipt as b2b_ecommerce_b2breceipt
