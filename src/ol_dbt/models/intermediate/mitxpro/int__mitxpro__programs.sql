...
with
    programs as (select * from {{ ref("stg__mitxpro__app__postgres__courses_program") }}),
    cms_programs as (select * from {{ ref("stg__mitxpro__app__postgres__cms_programpage") }}),
    ...
    course_program as (select * from {{ ref("int__mitxpro__coursesinprogram") }}),
    course_topics as (select * from {{ ref("int__mitxpro__courses_to_topics") }}),
    ...
    program_topics as (
        select course_program.program_id, array_join(array_agg(course_topics.coursetopic_name), ', ') as program_topics
        from course_topics
    ...
    program_instructors as (
        select program_id, array_join(array_agg(cms_facultymemberspage_facultymember_name), ', ') as program_instructors
...
from programs
left join cms_programs on programs.program_id = cms_programs.program_id
left join wagtail_page on cms_programs.wagtail_page_id = wagtail_page.wagtail_page_id
left join platform on programs.platform_id = platform.platform_id
