with
    b2borders as (select * from {{ ref("stg__mitxpro__app__postgres__b2becommerce_b2border") }}),
    productversions as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_productversion") }}),
    products as (select * from {{ ref("int__mitxpro__ecommerce_product") }}),
    salesforce_opportunity as (select * from {{ ref("int__salesforce__opportunity") }}),
    couponpaymentversion as (select * from {{ ref("int__mitxpro__ecommerce_couponpaymentversion") }}),
    salesforce_b2border as (
        select b2borders.b2border_id, salesforce_opportunity.opportunity_id
        from b2borders
        inner join
            couponpaymentversion on b2borders.couponpaymentversion_id = couponpaymentversion.couponpaymentversion_id
        inner join
