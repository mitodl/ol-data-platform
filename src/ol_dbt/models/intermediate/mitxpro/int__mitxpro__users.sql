with
    ...
    users_legaladdress as (select * from {{ ref("stg__mitxpro__app__postgres__users_legaladdress") }}),
    users_profile as (select * from {{ ref("stg__mitxpro__app__postgres__users_profile") }}),
    openedx_users as (select * from {{ ref("stg__mitxpro__openedx__mysql__auth_user") }}),
    company as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_company") }}),
    ...
    user_company as (
        select user_company_from_enrollments.user_id, company.company_name
...
left join openedx_users on users.user_username = openedx_users.user_username
