with
    productcouponassignment as (
        select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_productcouponassignment") }}
    ),
    couponproduct as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_couponproduct") }})
