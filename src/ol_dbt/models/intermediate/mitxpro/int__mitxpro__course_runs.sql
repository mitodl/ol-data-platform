...
with
    course_runs as (select * from {{ ref("stg__mitxpro__app__postgres__courses_courserun") }}),
    courses as (select * from {{ ref("stg__mitxpro__app__postgres__courses_course") }}),
    platforms as (select * from {{ ref("stg__mitxpro__app__postgres__courses_platform") }})
...
left join platforms on courses.platform_id = platforms.platform_id
