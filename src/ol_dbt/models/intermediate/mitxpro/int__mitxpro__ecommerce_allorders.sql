with
    b2becommerce_b2border as (select * from {{ ref("int__mitxpro__b2becommerce_b2border") }}),
    b2becommerce_b2breceipt as (select * from {{ ref("int__mitxpro__b2becommerce_b2breceipt") }}),
    b2becommerce_b2bcouponredemption as (select * from {{ ref("int__mitxpro__b2becommerce_b2bcouponredemption") }}),
    ecommerce_couponpaymentversion as (select * from {{ ref("int__mitxpro__ecommerce_couponpaymentversion") }}),
    ecommerce_couponredemption as (select * from {{ ref("int__mitxpro__ecommerce_couponredemption") }}),
    ...
    course_runs as (select * from {{ ref("int__mitxpro__course_runs") }}),
    programs as (select * from {{ ref("int__mitxpro__programs") }}),
    productversion as (select * from {{ ref("int__mitxpro__ecommerce_productversion") }}),
    reg_order_fields as (
        select
        ...
        inner join ecommerce_line on ecommerce_order.order_id = ecommerce_line.order_id
        left join
        ...
        left join ecommerce_couponredemption on ecommerce_order.order_id = ecommerce_couponredemption.order_id
        left join users on ecommerce_order.order_purchaser_user_id = users.user_id
        left join course_runs on ecommerce_line.courserun_id = course_runs.courserun_id
        left join programs on ecommerce_line.program_id = programs.program_id
        left join productversion on ecommerce_line.productversion_id = productversion.productversion_id
        left join
            b2becommerce_b2border
            on ecommerce_couponpaymentversion.couponpaymentversion_id = b2becommerce_b2border.couponpaymentversion_id
    ),
    reg_order_test as (select b2border_id from reg_order_fields group by b2border_id),
    b2b_order_fields as (
        select
            ecommerce_order.order_id,
            b2becommerce_b2border.b2border_id,
        ...
        left join ecommerce_line on ecommerce_order.order_id = ecommerce_line.order_id
        left join course_runs on ecommerce_line.courserun_id = course_runs.courserun_id
        left join programs on ecommerce_line.program_id = programs.program_id
        left join b2becommerce_b2breceipt on b2becommerce_b2border.b2border_id = b2becommerce_b2breceipt.b2border_id
        left join productversion on b2becommerce_b2border.productversion_id = productversion.productversion_id
        left join
            b2becommerce_b2bcouponredemption
        ...
        left join reg_order_test on b2becommerce_b2border.b2border_id = reg_order_test.b2border_id
