with
    coupon as (select * from {{ ref("int__mitxpro__ecommerce_coupon") }}),
    ecommerce_couponversion as (select * from {{ ref("int__mitxpro__ecommerce_couponversion") }}),
    ecommerce_couponpaymentversion as (select * from {{ ref("int__mitxpro__ecommerce_couponpaymentversion") }}),
    b2becommerce_b2bcoupon as (select * from {{ ref("int__mitxpro__b2becommerce_b2bcoupon") }}),
    ...
    reg_coupon_fields as (
        ...
        from coupon
        inner join latest_couponversion on coupon.coupon_id = latest_couponversion.coupon_id
        inner join
            ecommerce_couponversion
            on latest_couponversion.coupon_id = ecommerce_couponversion.coupon_id
