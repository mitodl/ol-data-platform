with
    products as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_product") }}),
    contenttypes as (select * from {{ ref("stg__mitxpro__app__postgres__django_contenttype") }}),
    courseruns as (select * from {{ ref("stg__mitxpro__app__postgres__courses_courserun") }}),
        ...
        from
            (
                select
                    productversion.*,
                    row_number() over (
                        partition by productversion.product_id order by productversion.productversion_updated_on desc
                    ) as row_num
    ...
    product_subquery as (
        select
            ...
            case
                contenttypes.contenttype_full_name when 'courses_program' then products.product_object_id
            end as program_id,
            case
