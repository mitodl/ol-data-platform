with
    orders as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_order") }}),
    couponredemption as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_couponredemption") }}),
    couponversion as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_couponversion") }}),
    coupon as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_coupon") }}),
    couponpaymentversion as (select * from {{ ref("stg__mitxpro__app__postgres__ecommerce_couponpaymentversion") }}),
    receipts as (select * from {{ ref("int__mitxpro__ecommerce_receipt") }} where receipt_transaction_status != 'ERROR')
...
select
    ...
    case
        when orders.order_state in ('fulfilled', 'refunded') then couponredemption.couponredemption_created_on
