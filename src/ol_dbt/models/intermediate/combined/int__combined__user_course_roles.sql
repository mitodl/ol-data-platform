with
    mitxonline_courseroles as (select * from {{ ref("stg__mitxonline__openedx__mysql__user_courseaccessrole") }}),
    mitxonline_app_users as (select * from {{ ref("stg__mitxonline__app__postgres__users_user") }}),
    mitxonline_openedxuser_mapping as (select * from {{ ref("stg__mitxonline__app__postgres__openedx_openedxuser") }}),
    mitxonline_openedx_users as (select * from {{ ref("stg__mitxonline__openedx__mysql__auth_user") }}),
    mitxpro_courseroles as (select * from {{ ref("stg__mitxpro__openedx__mysql__user_courseaccessrole") }}),
    mitxpro_app_users as (select * from {{ ref("stg__mitxpro__app__postgres__users_user") }}),
    mitxpro_openedx_users as (select * from {{ ref("stg__mitxpro__openedx__mysql__auth_user") }}),
    residential_courseroles as (select * from {{ ref("stg__mitxresidential__openedx__user_courseaccessrole") }}),
    residential_openedx_users as (select * from {{ ref("stg__mitxresidential__openedx__auth_user") }}),
    edxorg_courseroles as (select * from {{ ref("stg__edxorg__s3__user_courseaccessrole") }}),
    edxorg_users as (select * from {{ ref("stg__edxorg__s3__user") }}),
    edxorg_profiles as (select * from {{ ref("stg__edxorg__s3__user_profile") }}),
    user_course_role_seed_file as (select * from {{ ref("user_course_roles") }}),
    combined_courseroles as (
        ...
        left join
            mitxonline_openedxuser_mapping
            on mitxonline_openedx_users.user_username = mitxonline_openedxuser_mapping.openedxuser_username
        left join mitxonline_app_users on mitxonline_openedxuser_mapping.user_id = mitxonline_app_users.user_id
        ...
        inner join edxorg_users on edxorg_courseroles.user_id = edxorg_users.user_id
        left join edxorg_profiles on edxorg_courseroles.user_id = edxorg_profiles.user_id
        ...
        inner join mitxpro_openedx_users on mitxpro_courseroles.openedx_user_id = mitxpro_openedx_users.openedx_user_id
        left join
            mitxpro_app_users
            on mitxpro_openedx_users.user_username = mitxpro_app_users.user_username
...
left join
    combined_courseroles
    on user_course_role_seed_file.platform = combined_courseroles.platform
...
left join seed_file_users on user_course_role_seed_file.hashed_user_email = seed_file_users.hashed_user_email
