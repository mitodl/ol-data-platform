with
    mitx_enrollments as (select * from {{ ref("int__mitx__courserun_enrollments") }}),
    mitxpro_enrollments as (select * from {{ ref("int__mitxpro__courserunenrollments") }}),
    emeritus_enrollments as (select * from {{ ref("stg__emeritus__api__bigquery__user_enrollments") }}),
    global_alumni_enrollments as (select * from {{ ref("stg__global_alumni__api__bigquery__user_enrollments") }}),
    bootcamps_enrollments as (select * from {{ ref("int__bootcamps__courserunenrollments") }}),
    residential_enrollments as (select * from {{ ref("int__mitxresidential__courserun_enrollments") }}),
    residential_grades as (select * from {{ ref("int__mitxresidential__courserun_grades") }}),
    mitx_grades as (select * from {{ ref("int__mitx__courserun_grades") }}),
    mitxpro_grades as (select * from {{ ref("int__mitxpro__courserun_grades") }}),
    combined_certificates as (select * from {{ ref("int__combined__courserun_certificates") }}),
    combined_courseruns as (select * from {{ ref("int__combined__course_runs") }}),
    mitxpro_courseruns as (select * from {{ ref("int__mitxpro__course_runs") }}),
    combined_enrollments as (
        ...
        left join
            mitx_grades
            on mitx_enrollments.courserun_readable_id = mitx_grades.courserun_readable_id
        ...
        left join
            mitxpro_grades
            on mitxpro_enrollments.courserun_readable_id = mitxpro_grades.courserun_readable_id
        ...
        select
            ...
            coalesce(
                mitxpro_courseruns.courserun_readable_id, emeritus_enrollments.courserun_external_readable_id
        ...
        left join
            mitxpro_courseruns
            on emeritus_enrollments.courserun_external_readable_id = mitxpro_courseruns.courserun_external_readable_id
        ...
        select
            ...
            coalesce(
                mitxpro_courseruns.courserun_readable_id, global_alumni_enrollments.courserun_external_readable_id
            ) as courserun_readable_id,
            null as user_username,
        ...
        left join
            residential_grades
            on residential_enrollments.courserun_readable_id = residential_grades.courserun_readable_id
...
left join
    ...
    on combined_enrollments.platform = combined_certificates.platform
