...
with
    ...
    mitxpro_certificates as (
        select * from {{ ref("int__mitxpro__courserun_certificates") }} where courseruncertificate_is_revoked = false
    ),
    bootcamps_certificates as (
        select * from {{ ref("int__bootcamps__courserun_certificates") }} where courseruncertificate_is_revoked = false
    )
