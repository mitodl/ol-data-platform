with
    ...
    mitxpro_runs as (select * from {{ ref("int__mitxpro__course_runs") }}),
    bootcamps_courses as (select * from {{ ref("int__bootcamps__courses") }}),
    bootcamps_runs as (select * from {{ ref("int__bootcamps__course_runs") }}),
    ...
    residential_runs as (
        select *, {{ extract_course_readable_id("courserun_readable_id") }} as course_readable_id
    ...
    combined_runs as (
        ...
        left join
            micromasters_runs on edxorg_runs.courserun_readable_id = micromasters_runs.courserun_edxorg_readable_id

        union all

        select
            case
                when mitxpro_runs.platform_id = 2 then '{{ var("mitxpro") }}' else 'xPRO ' || mitxpro_runs.platform_name
            end as platform,
        ...
        from emeritus_runs
        left join
            mitxpro_runs on emeritus_runs.courserun_external_readable_id = mitxpro_runs.courserun_external_readable_id
        ...
        select
            ...
            null as courserun_is_live,
            case
                when cardinality(split(courserun_readable_id, '+')) >= 2 then split(courserun_readable_id, '+')[2]
            end as course_number,
            case
                when courserun_end_on is null and from_iso8601_timestamp(courserun_start_on) <= current_date
                then true
                when
