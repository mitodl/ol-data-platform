...
with
    ...
    user_info_combo as (
        select user_id, courserunenrollment_courserun_readable_id
        from {{ ref("stg__edxorg__bigquery__mitx_user_info_combo") }}
        where courserun_platform = '{{ var("edxorg") }}' and courserunenrollment_courserun_readable_id is not null
    ),
    micromasters_runs as (select * from {{ ref("stg__micromasters__app__postgres__courses_courserun") }}),
    edxorg_enrollments as (
        ...
        inner join
            user_info_combo
            on person_courses.user_id = user_info_combo.user_id
            and person_courses.courserun_readable_id = user_info_combo.courserunenrollment_courserun_readable_id
        left join
            micromasters_runs on person_courses.courserun_readable_id = micromasters_runs.courserun_edxorg_readable_id
    ),
    edxorg_runs as (select * from {{ ref("stg__edxorg__bigquery__mitx_courserun") }}),
    ...
    micromasters_courses as (select * from {{ ref("stg__micromasters__app__postgres__courses_course") }}),
    micromasters_users as (select * from {{ ref("__micromasters__users") }}),
    dedp_edxorg_enrollments_verified as (
        select distinct edxorg_enrollments.user_id, edxorg_enrollments.courserun_readable_id
        ...
        inner join
            micromasters_lines
            on edxorg_enrollments.courserun_readable_id = micromasters_lines.courserun_edxorg_readable_id
            and micromasters_orders.order_id = micromasters_lines.order_id
        inner join
            micromasters_runs on micromasters_lines.courserun_readable_id = micromasters_runs.courserun_readable_id
    ...
    enrollments as (
        select
            ...
            case
                when
                    ...
                    and dedp_edxorg_enrollments_verified.courserun_readable_id is not null
                then 'verified'
        ...
        left join
            dedp_edxorg_enrollments_verified
            on edxorg_enrollments.user_id = dedp_edxorg_enrollments_verified.user_id
