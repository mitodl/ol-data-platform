with
    course_activities as (select * from {{ ref("int__edxorg__mitx_user_activity") }}),
    course_activities_video as (select * from {{ ref("int__edxorg__mitx_user_courseactivity_video") }}),
    problem_check as (select * from {{ ref("int__edxorg__mitx_user_courseactivity_problemcheck") }}),
    ...
    all_course_activities_stats as (
        ...
        group by course_activities.user_username, course_activities.courserun_readable_id
    )
