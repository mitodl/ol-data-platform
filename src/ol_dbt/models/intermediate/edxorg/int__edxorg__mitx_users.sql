...
with
    ...
    most_recent_user_info as (
        select user_id, user_email, user_username, user_full_name from email_opt_in where row_num = 1
    ),
    combined_user_info as (
        select
            ...
            row_number() over (
                partition by user_info_combo.user_id order by user_info_combo.user_last_login desc
            ) as row_num
        from user_info_combo
        left join most_recent_user_info on user_info_combo.user_id = most_recent_user_info.user_id
    ),
    users as (
        select
            user_id,
