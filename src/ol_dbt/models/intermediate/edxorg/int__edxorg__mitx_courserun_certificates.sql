...
with
    person_courses as (
        ...
        where courserun_platform = '{{ var("edxorg") }}' and courseruncertificate_is_earned = true
    ),
    user_info_combo as (
        select *
        from {{ ref("stg__edxorg__bigquery__mitx_user_info_combo") }}
        where courserun_platform = '{{ var("edxorg") }}' and courseruncertificate_id is not null
    ),
    certificates as (
        select
        ...
        inner join
            person_courses  -- this join is needed to filter out courseruncertificate that are not earned
            on user_info_combo.user_id = person_courses.user_id
            and user_info_combo.courseruncertificate_courserun_readable_id = person_courses.courserun_readable_id
    ),
    users as (select * from {{ ref("int__edxorg__mitx_users") }}),
    runs as (select * from {{ ref("int__edxorg__mitx_courseruns") }}),
    micromasters_users as (select * from {{ ref("__micromasters__users") }}),
    edxorg_enrollments as (select * from {{ ref("int__edxorg__mitx_courserun_enrollments") }}),
    ...
    edxorg_dedp_certificates_from_micromasters as (
        ...
        left join
            runs on dedp_edxorg_certificates_from_micromasters.courserun_edxorg_readable_id = runs.courserun_readable_id
        left join
            dedp_edxorg_grades_from_micromasters
            on dedp_edxorg_certificates_from_micromasters.courserun_readable_id
        ...
        left join
            edxorg_enrollments
            on dedp_edxorg_certificates_from_micromasters.courserun_edxorg_readable_id
    ...
    edxorg_non_dedp_certificates as (
        ...
        left join runs on certificates.courserun_readable_id = runs.courserun_readable_id
        left join micromasters_users on certificates.user_username = micromasters_users.user_edxorg_username
        where
