...
with
    runs_from_bigquery as (
        select *
        from {{ ref("stg__edxorg__bigquery__mitx_courserun") }}
        where courserun_platform = '{{ var("edxorg") }}'
    ),
    courseruns as (select * from {{ ref("stg__edxorg__api__courserun") }}),
    courses as (select * from {{ ref("stg__edxorg__api__course") }}),
    instructors as (
        select
    ...
    runs_from_api as (
        ...
        from courseruns
        inner join courses on courseruns.course_readable_id = courses.course_readable_id
        inner join instructors on courseruns.courserun_readable_id = instructors.courserun_readable_id
    ),
    runs as (
        select
            ...
            coalesce(
                runs_from_api.courserun_enrollment_start_on, runs_from_bigquery.courserun_enrollment_start_date
        ...
        full outer join
            runs_from_bigquery
            on replace(replace(runs_from_api.courserun_readable_id, 'course-v1:', ''), '+', '/')
            = runs_from_bigquery.courserun_readable_id
    ),
    ...
    micromasters_courses as (
        select course_id, program_id, course_edx_key, course_number, course_edx_key as course_readable_id
...
from runs
left join micromasters_courses on runs.course_number = micromasters_courses.course_number
