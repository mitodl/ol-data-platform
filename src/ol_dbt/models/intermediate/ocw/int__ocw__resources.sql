with
    websites as (select * from {{ ref("stg__ocw__studio__postgres__websites_website") }}),
    websitecontents as (select * from {{ ref("stg__ocw__studio__postgres__websites_websitecontent") }}),
    websitestarters as (select * from {{ ref("stg__ocw__studio__postgres__websites_websitestarter") }}),
    sitemetadata as (
        select
            website_uuid,
...
select
    ...
    nullif(
        nullif(json_query(websitecontents.websitecontent_metadata, 'lax $.url_status_code' omit quotes), ''), 'null'
    ) as external_resource_url_status_code,
    nullif(
        nullif(json_query(websitecontents.websitecontent_metadata, 'lax $.backup_url_status_code' omit quotes), ''),
        'null'
    ...
    || '/' as studio_url,
    nullif(
        nullif(json_query(websitecontents.websitecontent_metadata, 'lax $.backup_url' omit quotes), ''), 'null'
    ...
    nullif(
        nullif(
            json_query(websitecontents.websitecontent_metadata, 'lax $.video_metadata.youtube_description' omit quotes),
            ''
        ),
        'null'
    ) as video_youtube_description,
    nullif(
        nullif(json_query(websitecontents.websitecontent_metadata, 'lax $.video_metadata.youtube_id' omit quotes), ''),
        'null'
    ) as video_youtube_id,
    nullif(
        nullif(
            json_query(websitecontents.websitecontent_metadata, 'lax $.video_metadata.video_speakers' omit quotes), ''
        ),
        'null'
    ) as video_youtube_speakers,
    nullif(
        nullif(json_query(websitecontents.websitecontent_metadata, 'lax $.video_metadata.video_tags' omit quotes), ''),
        'null'
    ) as video_youtube_tags,
    -- video_files for video resources
    nullif(
        nullif(json_query(websitecontents.websitecontent_metadata, 'lax $.video_files.archive_url' omit quotes), ''),
        'null'
    ) as video_archive_url,
    nullif(
        nullif(
            json_query(websitecontents.websitecontent_metadata, 'lax $.video_files.video_captions_file' omit quotes), ''
        ),
        'null'
    ) as video_captions_file,
    nullif(
        nullif(
            json_query(websitecontents.websitecontent_metadata, 'lax $.video_files.video_thumbnail_file' omit quotes),
            ''
        ),
        'null'
    ...
    nullif(
        nullif(
            json_query(websitecontents.websitecontent_metadata, 'lax $.video_files.video_transcript_file' omit quotes),
            ''
        ),
        'null'
    ) as video_transcript_file
from websites
inner join websitecontents on websites.website_uuid = websitecontents.website_uuid
left join sitemetadata on websites.website_uuid = sitemetadata.website_uuid
inner join websitestarters on websites.websitestarter_id = websitestarters.websitestarter_id
where
