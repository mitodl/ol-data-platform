with
    course_certificates_dedp_from_micromasters as (
        select * from {{ ref("__micromasters_course_certificates_dedp_from_micromasters") }}
    ),
    course_certificates_dedp_from_mitxonline as (
        select * from {{ ref("__micromasters_course_certificates_dedp_from_mitxonline") }}
    ),
    course_certificates_non_dedp_program as (
        select * from {{ ref("__micromasters_course_certificates_non_dedp_from_edxorg") }}
    ),
    ...
    dedp_course_certificates_combined as (
        ...
        left join
            mitx_users on course_certificates_dedp_from_mitxonline.user_mitxonline_id = mitx_users.user_mitxonline_id
    ...
    dedp_course_certificates as (select * from dedp_course_certificates_sorted where row_num = 1),
    non_dedp_course_certificates as (
        ...
        left join mitx_users on course_certificates_non_dedp_program.user_edxorg_id = mitx_users.user_edxorg_id
    ),
