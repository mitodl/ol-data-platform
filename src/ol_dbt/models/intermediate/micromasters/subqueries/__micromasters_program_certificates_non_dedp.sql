...
with
    ...
    edx_users as (select * from {{ ref("int__edxorg__mitx_users") }}),
    programs as (select * from {{ ref("int__mitx__programs") }} where is_dedp_program = false),
    program_certificates_override_list as (
        select * from {{ ref("stg__micromasters__app__user_program_certificate_override_list") }}
    ),
    non_dedp_certificates as (
        ...
        left join edx_users on micromasters_program_certificates.user_id = edx_users.user_id
        left join
            programs on micromasters_program_certificates.micromasters_program_id = programs.micromasters_program_id
    ...
    non_dedp_overides as (
        ...
        inner join edx_users on program_certificates_override_list.user_edxorg_id = edx_users.user_id
        inner join
            programs on program_certificates_override_list.micromasters_program_id = programs.micromasters_program_id
        left join
            non_dedp_certificates
            on program_certificates_override_list.user_edxorg_id = non_dedp_certificates.user_edxorg_id
            and program_certificates_override_list.micromasters_program_id
