with
    users as (select * from {{ ref("stg__micromasters__app__postgres__auth_user") }}),
    profiles as (select * from {{ ref("stg__micromasters__app__postgres__profiles_profile") }}),
    mitxonline_auth as (
        select user_id, user_username as user_mitxonline_username
        from {{ ref("stg__micromasters__app__postgres__auth_usersocialauth") }}
        where user_auth_provider = 'mitxonline'
    ),
    edxorg_auth as (
        select user_id, user_username as user_edxorg_username
        from {{ ref("stg__micromasters__app__postgres__auth_usersocialauth") }}
        where user_auth_provider = 'edxorg'
    ),
    edxorg_users as (
        select user_username as user_edxorg_username, user_last_login from {{ ref("int__edxorg__mitx_users") }}
    ),
    most_recent_edx_username as (
        select user_id, user_edxorg_username
        from
            (
                select
                    edxorg_auth.user_id,
                    ...
                    row_number() over (
                        partition by edxorg_auth.user_id order by edxorg_users.user_last_login desc
                    ) as row_num
                from edxorg_auth
