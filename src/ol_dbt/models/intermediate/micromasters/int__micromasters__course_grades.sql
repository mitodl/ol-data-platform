with
    course_grades_dedp_from_micromasters as (
        select * from {{ ref("__micromasters_course_grades_dedp_from_micromasters") }}
    ),
    course_grades_dedp_from_mitxonline as (
        select * from {{ ref("__micromasters_course_grades_dedp_from_mitxonline") }}
    ),
    course_grades_non_dedp_program as (select * from {{ ref("__micromasters_course_grades_non_dedp_from_edxorg") }}),
    ...
    dedp_course_grades_combined as (
        ...
        left join
            mitx_users on course_grades_dedp_from_micromasters.user_micromasters_id = mitx_users.user_micromasters_id
        ...
        left join mitx_users on course_grades_dedp_from_mitxonline.user_mitxonline_id = mitx_users.user_mitxonline_id
    ...
    dedp_course_grades_sorted as (
        select
            ...
            is_passing,
            case
                when user_mitxonline_username is not null
                then
                    row_number() over (
    ...
    dedp_course_grades as (select * from dedp_course_grades_sorted where row_num = 1),
    non_dedp_course_grades as (
        ...
        left join mitx_users on course_grades_non_dedp_program.user_edxorg_id = mitx_users.user_edxorg_id
    ),
