with
    program_certificates_dedp_from_micromasters as (
        select * from {{ ref("__micromasters_program_certificates_dedp_from_micromasters") }}
    ),
    program_certificates_dedp_from_mitxonline as (
        select * from {{ ref("__micromasters_program_certificates_dedp_from_mitxonline") }}
    ),
    program_certificates_non_dedp as (select * from {{ ref("__micromasters_program_certificates_non_dedp") }}),
    ...
    report as (
        ...
        left join
            mitx_users on program_certificates_dedp_from_mitxonline.user_mitxonline_id = mitx_users.user_mitxonline_id
        ...
        left join mitx_users on program_certificates_non_dedp.user_edxorg_id = mitx_users.user_edxorg_id
    )
...
from report
left join mitxonline_programs on report.mitxonline_program_id = mitxonline_programs.program_id
