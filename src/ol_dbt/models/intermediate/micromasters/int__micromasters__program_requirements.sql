...
-- 4	           28	      Core                                                5
with
    programs as (select * from {{ ref("stg__micromasters__app__postgres__courses_program") }}),
    courses as (select * from {{ ref("stg__micromasters__app__postgres__courses_course") }}),
    electiveset as (select * from {{ ref("stg__micromasters__app__postgres__courses_electiveset") }}),
    ...
    elective_courses as (
        ...
        inner join electiveset on electiveset_to_course.electiveset_id = electiveset.electiveset_id
    ),
    -- - courses that are not in elective_courses are core
    core_courses as (
        select courses.course_id, courses.program_id
        from courses
        left join
            elective_courses
    ...
    combined_courses as (
        ...
        select electiveset_id, program_id, electiveset_required_number, course_id, 'Elective' as programrequirement_type
