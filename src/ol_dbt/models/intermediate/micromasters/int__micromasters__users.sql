with
    ...
    highest_education as (select user_profile_id, user_education_degree from education where row_num = 1),
    employment as (
        select
            user_profile_id,
            user_company_name,
    ...
    most_recent_employment as (
        select user_profile_id, user_company_name, user_job_position, user_company_industry
        from employment
        where row_num = 1
    )
