...
with
    micromasters_program_enrollments as (
        -- - There are learners who received both 'Statistics and Data Science (General track)' and 'Statistics and Data
        -- Science' from 2U data, but we only count them once in 'Statistics and Data Science' for MM program
        -- enrollments
        -- report.
        select *, row_number() over (partition by user_id, micromasters_program_id order by program_title) as row_num
    ...
    mitxonline_programenrollments as (select * from {{ ref("int__mitxonline__programenrollments") }}),
    programs as (select * from {{ ref("int__mitx__programs") }}),
    mitx_users as (select * from {{ ref("int__mitx__users") }}),
    mitxonline_dedp_records as (
        ...
        inner join programs on mitxonline_programenrollments.program_id = programs.mitxonline_program_id
        inner join mitx_users on mitxonline_programenrollments.user_id = mitx_users.user_mitxonline_id
    ...
    mm_dedp_records as (
        ...
        inner join programs on mm_program_enrollments.program_id = programs.micromasters_program_id
        inner join mitx_users on mm_program_enrollments.user_id = mitx_users.user_micromasters_id
        left join
            mitxonline_dedp_records
            on mitx_users.user_micromasters_email = mitxonline_dedp_records.user_email
            and programs.program_title = mitxonline_dedp_records.program_title
        where programs.is_dedp_program = true and mitxonline_dedp_records.user_email is null
    ),
    non_dedp_records as (
        select
        ...
        inner join mitx_users on micromasters_program_enrollments.user_id = mitx_users.user_edxorg_id
        inner join
            programs on micromasters_program_enrollments.micromasters_program_id = programs.micromasters_program_id
        where micromasters_program_enrollments.row_num = 1 and programs.is_dedp_program = false
    )
