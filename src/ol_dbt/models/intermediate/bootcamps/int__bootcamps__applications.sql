with
    applications as (select * from {{ ref("stg__bootcamps__app__postgres__applications_courserun_application") }}),
    runs as (select * from {{ ref("stg__bootcamps__app__postgres__courses_courserun") }}),
    application_steps as (select * from {{ ref("stg__bootcamps__app__postgres__applications_applicationstep") }}),
    application_last_steps as (
        select course_id, max(applicationstep_step_order) as last_step from application_steps group by course_id
    ),
    ...
    personal_prices as (select * from {{ ref("stg__bootcamps__app__postgres__courses_personalprice") }}),
    installments as (
        select courserun_id, sum(installment_amount) as list_price
        from {{ ref("stg__bootcamps__app__postgres__courses_installment") }}
        group by courserun_id
    ),
    fulfilled_orders as (
        select application_id, sum(line_price) as total_paid
    ...
    courserun_last_steps as (
        ...
        from courserun_applicationstep
        inner join
            application_steps on courserun_applicationstep.applicationstep_id = application_steps.applicationstep_id
        inner join
            application_last_steps
            on application_steps.course_id = application_last_steps.course_id
...
left join courserun_last_steps on applications.courserun_id = courserun_last_steps.courserun_id
left join
    applicationstep_submissions
    on courserun_last_steps.courserun_applicationstep_id = applicationstep_submissions.courserun_applicationstep_id
    and applications.application_id = applicationstep_submissions.application_id
left join installments on applications.courserun_id = installments.courserun_id
left join
    personal_prices
    on applications.user_id = personal_prices.user_id
    and applications.courserun_id = personal_prices.courserun_id
left join fulfilled_orders on applications.application_id = fulfilled_orders.application_id
