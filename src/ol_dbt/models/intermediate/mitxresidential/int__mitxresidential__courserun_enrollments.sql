with
    enrollments as (select * from {{ ref("stg__mitxresidential__openedx__courserun_enrollment") }}),
    runs as (select * from {{ ref("stg__mitxresidential__openedx__courserun") }}),
    users as (select * from {{ ref("int__mitxresidential__users") }}),
