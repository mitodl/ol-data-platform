with
    exam_unit_grades as (
        select *
        from {{ ref("int__mitxonline__courserun_subsection_grades") }}
        where lower(coursestructure_block_title) = 'proctored exam' and subsectiongrade_total_graded_score > 0
    ),
    exam_courserun_grades as (
        select * from {{ ref("int__mitxonline__courserun_grades") }} where lower(courserun_title) like '%proctored exam'
    ),
    exam_courseruns_from_micromasters as (
        ...
        select examrun_readable_id, examrun_semester, examrun_passing_grade, course_id
        from {{ ref("stg__micromasters__app__postgres__exams_examrun") }}
    ),
    courses_from_micromasters as (select * from {{ ref("stg__micromasters__app__postgres__courses_course") }}),
    courseruns_from_mitxonline as (select * from {{ ref("stg__mitxonline__app__postgres__courses_courserun") }}),
    courses_from_mitxonline as (select * from {{ ref("stg__mitxonline__app__postgres__courses_course") }}),
...
left join
    exam_courseruns_from_micromasters
...
select
    ...
    exam_unit_grades.subsectiongrade_total_earned_graded_score
    / exam_unit_grades.subsectiongrade_total_graded_score as proctoredexamgrade_grade,
...
left join
    exam_courserun_grades
    on users.user_username = exam_courserun_grades.user_username
