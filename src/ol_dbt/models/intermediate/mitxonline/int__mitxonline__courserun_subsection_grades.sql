with
    subsection_grades as (select * from {{ ref("stg__mitxonline__openedx__mysql__grades_subsectiongrade") }}),
    ...
    subsection_grades_joined as (
        select
            ...
            if(
                subsection_grade_overrides.subsectiongradeoverride_id is not null, true, false
            ) as subsectiongrade_is_overridden
        from subsection_grades
        inner join
            course_structure on subsection_grades.coursestructure_block_id = course_structure.coursestructure_block_id
        left join
            subsection_grade_overrides
            on subsection_grades.subsectiongrade_id = subsection_grade_overrides.subsectiongrade_id
        where course_structure.row_num = 1
    )

select *
from subsection_grades_joined
