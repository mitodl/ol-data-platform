...
with
    enrollments as (select * from {{ ref("stg__mitxonline__app__postgres__courses_programenrollment") }}),
    programs as (select program_id, program_title from {{ ref("stg__mitxonline__app__postgres__courses_program") }}),
    users as (select user_id, user_username, user_email from {{ ref("int__mitxonline__users") }}),
