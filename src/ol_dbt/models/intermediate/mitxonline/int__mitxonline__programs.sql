...
with
    ...
    program_pages as (select * from {{ ref("stg__mitxonline__app__postgres__cms_programpage") }}),
    program_requirements as (select program_id, course_id from {{ ref("int__mitxonline__program_requirements") }}),
    wagtail_page as (select * from {{ ref("stg__mitxonline__app__postgres__cms_wagtail_page") }}),
    course_topics as (select * from {{ ref("int__mitxonline__course_to_topics") }}),
    program_topics as (
        select
    ...
    program_instructors as (
        select program_id, array_join(array_agg(instructor_name), ', ') as program_instructors
