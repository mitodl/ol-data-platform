...
with
    ...
    program_requirements as (select * from {{ ref("int__mitxonline__program_requirements") }}),
    programs as (select * from {{ ref("int__mitxonline__programs") }}),
    ...
    course_instructors as (
        select course_id, array_join(array_agg(instructor_name), ', ') as course_instructors
