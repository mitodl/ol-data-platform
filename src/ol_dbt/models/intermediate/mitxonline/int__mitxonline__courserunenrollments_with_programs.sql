with
    enrollments as (select * from {{ ref("int__mitxonline__courserunenrollments") }}),
    program_requirements as (select * from {{ ref("int__mitx__program_requirements") }}),
    ...
    main_query as (
        ...
        inner join programs on program_requirements.mitxonline_program_id = programs.mitxonline_program_id
    ...
    good_economics_for_hard_times_query as (
        ...
        from enrollments
        inner join
            mitxonline_good_economics_for_hard_times_programs
            on enrollments.courserunenrollment_id
            = mitxonline_good_economics_for_hard_times_programs.courserunenrollment_id
        inner join
            programs on mitxonline_good_economics_for_hard_times_programs.program_id = programs.mitxonline_program_id
        where enrollments.course_number = '{{ "dedp_mitxonline_good_economics_for_hard_times_course_number" }}'
    )
