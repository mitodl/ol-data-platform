with
    users as (select * from {{ ref("stg__mitxonline__app__postgres__users_user") }}),
    users_legaladdress as (select * from {{ ref("stg__mitxonline__app__postgres__users_legaladdress") }}),
    users_profile as (select * from {{ ref("stg__mitxonline__app__postgres__users_userprofile") }}),
    micromasters_profiles as (select * from {{ ref("stg__micromasters__app__postgres__profiles_profile") }}),
    ...
    micromasters_profile as (
        select micromasters_profiles.user_profile_id, micromasters_profiles.user_id, micromasters_auth.user_username
    ...
    openedx_users as (select * from {{ ref("stg__mitxonline__openedx__mysql__auth_user") }}),
    openedxuser_mapping as (select * from {{ ref("stg__mitxonline__app__postgres__openedx_openedxuser") }})
...
left join openedxuser_mapping on users.user_id = openedxuser_mapping.user_id
left join openedx_users on openedxuser_mapping.openedxuser_username = openedx_users.user_username
left join micromasters_profile on openedxuser_mapping.openedxuser_username = micromasters_profile.user_username
left join micromasters_users on micromasters_profile.user_profile_id = micromasters_users.user_profile_id
