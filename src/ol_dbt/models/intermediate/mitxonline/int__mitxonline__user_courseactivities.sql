with
    ...
    course_activities_video as (select * from {{ ref("int__mitxonline__user_courseactivity_video") }}),
    problem_check as (select * from {{ ref("int__mitxonline__user_courseactivity_problemcheck") }}),
    course_state as (select * from {{ ref("stg__mitxonline__openedx__mysql__courseware_studentmodule") }}),
    course_structure as (select * from {{ ref("int__mitxonline__course_structure") }}),
    course_chapters_stats as (
        ...
        inner join
            course_structure
            on course_state.courserun_readable_id = course_structure.courserun_readable_id
