with
    lines as (select * from {{ ref("stg__mitxonline__app__postgres__ecommerce_line") }}),
    contenttypes as (select * from {{ ref("stg__mitxonline__app__postgres__django_contenttype") }}),
    versions as (
        select *
    ...
    orders as (select * from {{ ref("stg__mitxonline__app__postgres__ecommerce_order") }}),
    users as (select * from {{ ref("int__mitxonline__users") }}),
    intermediate_products_view as (select * from {{ ref("int__mitxonline__ecommerce_product") }}),
    discounts as (select * from {{ ref("stg__mitxonline__app__postgres__ecommerce_discount") }}),
    ...
    discountredemptions as (
        select *, row_number() over (partition by order_id order by discountredemption_timestamp desc) as row_num
    ...
    payments as (
        select *, row_number() over (partition by order_id order by transaction_created_on desc) as row_num
...
select
    ...
    case
        when orders.order_state in ('fulfilled', 'refunded') then discountredemptions.discountredemption_timestamp
...
left join payments on orders.order_id = payments.order_id and payments.row_num = 1
left join discountredemptions on orders.order_id = discountredemptions.order_id and discountredemptions.row_num = 1
