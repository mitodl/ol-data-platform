with
    flexiblepriceapplication as (
        select * from {{ ref("stg__mitxonline__app__postgres__flexiblepricing_flexiblepriceapplication") }}
    ),
    flexiblepricetier as (select * from {{ ref("stg__mitxonline__app__postgres__flexiblepricing_flexiblepricetier") }}),
    discount as (select * from {{ ref("stg__mitxonline__app__postgres__ecommerce_discount") }}),
...
select
    ...
    case
        contenttypes.contenttype_full_name when 'courses_course' then flexiblepriceapplication.courseware_object_id
    end as course_id,
    case
        contenttypes.contenttype_full_name when 'courses_program' then flexiblepriceapplication.courseware_object_id
    end as program_id,
    case
        contenttypes.contenttype_full_name when 'courses_course' then 'course' when 'courses_program' then 'program'
