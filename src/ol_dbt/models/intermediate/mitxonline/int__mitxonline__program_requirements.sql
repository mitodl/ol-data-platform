...
with
    program_requirements as (select * from {{ ref("stg__mitxonline__app__postgres__courses_programrequirement") }}),
    operator_nodes as (select * from program_requirements where programrequirement_node_type = 'operator'),
    course_nodes as (select * from program_requirements where programrequirement_node_type = 'course'),
    all_requirements as (
        select
            ...
            operator_nodes.programrequirement_operator_value as electiveset_required_number,
            case
                operator_nodes.programrequirement_operator
                when 'min_number_of'
                then 'Elective'
                when 'all_of'
                then 'Core'
            end as programrequirement_type,
            row_number() over (
        ...
        from course_nodes
        left join
            operator_nodes on course_nodes.programrequirement_path like operator_nodes.programrequirement_path || '%'
    ),
    ...
    combined_requirements as (
        select
            ...
            parent_requirements.programrequirement_requirement_id
            is not null as programrequirement_is_a_nested_requirement
        from child_requirements
        left join
            parent_requirements
            on child_requirements.programrequirement_path like parent_requirements.programrequirement_path || '%'
    ...
    core_courses_count as (
        select program_id, count_if(programrequirement_type = 'Core') as program_num_core_courses
        from combined_requirements
        group by program_id
    ),
    elective_courses_count as (
        select program_id, sum(electiveset_required_number) as program_num_elective_courses
        from
            (
                select
                ...
                group by combined_requirements.program_id, combined_requirements.programrequirement_requirement_id
            )
        group by program_id
