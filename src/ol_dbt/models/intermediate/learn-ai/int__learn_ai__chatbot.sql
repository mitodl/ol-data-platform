with
    chatsession as (select * from {{ ref("stg__learn_ai__app__postgres__chatbots_userchatsession") }}),
    djangocheckpoint as (select * from {{ ref("stg__learn_ai__app__postgres__chatbots_djangocheckpoint") }}),
    users as (select * from {{ ref("stg__learn_ai__app__postgres__users_user") }}),
    video as (
        select distinct
    ...
    videos_with_ranking as (
        select
            ...
            row_number() over (
                partition by chatsession.chatsession_object_id order by video.retrieved_at asc
            ) as row_num
        from chatsession
        inner join
            video
            on chatsession.chatsession_object_id like '%' || video.transcript_id
            and video.courserun_readable_id
            like '%'
            || substring(
                replace(chatsession.chatsession_object_id, 'asset-v1:', ''),
                1,
...
left join
    videos_with_ranking
    on chatsession.chatsession_object_id = videos_with_ranking.chatsession_object_id
