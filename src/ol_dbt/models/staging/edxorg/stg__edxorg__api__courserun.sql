with
    source as (select * from {{ source("ol_warehouse_raw_data", "raw__edxorg__s3__mitx_course_run") }})
    ...
    cleaned as (
        select
            ...
            case
                when pacing_type = 'self_paced' then true when pacing_type = 'instructor_paced' then false
            end as courserun_is_self_paced,
            case
                when weeks_to_complete = 1
                then cast(weeks_to_complete as varchar) || ' week'
                when weeks_to_complete > 1
                then cast(weeks_to_complete as varchar) || ' weeks'
