with
    source as (select * from {{ source("ol_warehouse_raw_data", "raw__edxorg__program_learner_report") }}),
    source_sorted as (
        select
            *,
            case
                when "date program certificate awarded" = 'null'
                then null  -- noqa: ST10
                else
    ...
    dedup_source as (select * from source_sorted where row_num = 1),
    cleaned as (
        ...
        select
            ...
            case
                when "program uuid" like '%941d3eaf56966c7'
                then 'Finance'
                when "program uuid" like '%3173ff51e11a748'
                then 'MIT Finance'
                when "program uuid" like '%8c11bfd9c0d7b07'
                then 'Statistics and Data Science (General Track)'
                when "program uuid" like '%cd7c6461dd9b1d4'
...
from cleaned
left join
    aggregated_program_certificate
    on cleaned.user_id = aggregated_program_certificate.user_id
