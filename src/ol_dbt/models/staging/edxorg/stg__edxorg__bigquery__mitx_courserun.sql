...
with
    source as (select * from {{ source("ol_warehouse_raw_data", "raw__irx__edxorg__bigquery__mitx_course") }}),
    renamed as (
        ...
        select
            ...
            case
                course_id
                when 'MITx/14.74x/3T2015'
                then '14.740x'
                when 'MITx/ESD.SCM1x/3T2014'
                then 'CTL.SC1x'
                when 'MITx/6.041x_3/2T2016'
                then '6.431x'
                when 'MITx/6.041x_4/1T2017'
                then '6.431x'
                else replace(course_number, ' ', '')
            end as course_number,
            case when institution = 'MITx_PRO' then 'MITxPRO' else institution end as courserun_institution,
            course_id as courserun_readable_id,
            concat(
                element_at(split(course_id, '/'), 1), '/', element_at(split(course_id, '/'), 2)
