with
    ...
    cleaned as (
        select
            ...
            case
                when lower(course_start) = 'null' then null else {{ cast_timestamp_to_iso8601("course_start") }}
            end as courserun_start_on,
            case
                when lower(course_end) = 'null' then null else {{ cast_timestamp_to_iso8601("course_end") }}
            end as courserun_end_on,
            case
                when lower(enrollment_start) = 'null' then null else {{ cast_timestamp_to_iso8601("enrollment_start") }}
            end as courserun_enrollment_start_on,
            case
                when lower(enrollment_end) = 'null' then null else {{ cast_timestamp_to_iso8601("enrollment_end") }}
