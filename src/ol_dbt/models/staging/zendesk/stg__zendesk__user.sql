with source as (
    select * from {{ source('ol_warehouse_raw_data','raw__thirdparty__zendesk_support__users') }}
)

{{ deduplicate_raw_table(raw_table='raw__thirdparty__zendesk_support__users', partition_columns = 'id') }}
, cleaned as (
    select
        id as user_id
        , url as user_api_url
        ...
        , role as user_role
        , case
            when role_type = 0 then 'custom agent'
