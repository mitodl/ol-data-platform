with source as (
    select * from {{ source('ol_warehouse_raw_data','raw__thirdparty__zendesk_support__ticket_comments') }}
)

{{ deduplicate_raw_table(raw_table='raw__thirdparty__zendesk_support__ticket_comments', partition_columns = 'id') }}
, cleaned as (
    select
        id as comment_id
        , ticket_id
