-- MITx Online ProgramEnrollment audit log (before/after change tracking)

with source as (
    select * from {{ source('ol_warehouse_raw_data', 'raw__mitxonline__app__postgres__courses_programenrollmentaudit') }}
)

{{ deduplicate_raw_table(raw_table='raw__mitxonline__app__postgres__courses_programenrollmentaudit', partition_columns='id') }}

, cleaned as (

    select
        id as enrollmentaudit_id
        , enrollment_id as programenrollment_id
        , acting_user_id
