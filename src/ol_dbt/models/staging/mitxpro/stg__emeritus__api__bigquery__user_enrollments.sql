with
    source as (select * from {{ source("ol_warehouse_raw_data", "raw__emeritus__bigquery__api_enrollments") }})

    {{
        deduplicate_raw_table(
            order_by="opportunity_last_modified_date", partition_columns="batch_id, email, first_name, last_name"
        )
    }},
    cleaned as (
        select
            ...
            case
                when gender = 'Prefer not to answer'
