with
    coursepage as (select * from {{ source("ol_warehouse_raw_data", "raw__xpro__app__postgres__cms_coursepage") }}),
    coursepage_sorted as (
        select *, row_number() over (partition by course_id order by _airbyte_extracted_at desc) as row_num
        from coursepage
    ),
    most_recent_coursepage as (select * from coursepage_sorted where row_num = 1),
    ...
    most_recent_externalcoursepage as (select * from externalcoursepage_sorted where row_num = 1)
...
left join coursepage on most_recent_externalcoursepage.course_id = coursepage.course_id
