with c as (
    select * from {{ source('ol_warehouse_raw_data','raw__xpro__openedx__mysql__assessment_criterion') }}
)

, co as (
    select * from {{ source('ol_warehouse_raw_data','raw__xpro__openedx__mysql__assessment_criterionoption') }}
)

, rub as (
    select * from {{ source('ol_warehouse_raw_data','raw__xpro__openedx__mysql__assessment_rubric') }}
)
...
, aigw as (
    select * from {{ source('ol_warehouse_raw_data','raw__xpro__openedx__mysql__assessment_aigradingworkflow') }}
)

, acs as (
    select * from {{ source('ol_warehouse_raw_data','raw__xpro__openedx__mysql__assessment_aiclassifierset') }}
)
