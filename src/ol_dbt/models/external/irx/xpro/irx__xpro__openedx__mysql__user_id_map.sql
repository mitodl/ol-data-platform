with
    student_anonymoususerid as (
        select * from {{ source("ol_warehouse_raw_data", "raw__xpro__openedx__mysql__student_anonymoususerid") }}
    ),
    auth_user as (select * from {{ source("ol_warehouse_raw_data", "raw__xpro__openedx__mysql__auth_user") }}),
