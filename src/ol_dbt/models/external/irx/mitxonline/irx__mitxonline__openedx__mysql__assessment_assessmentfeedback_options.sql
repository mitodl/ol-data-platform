with
    ...
    af as (
        select *
        from {{ source("ol_warehouse_raw_data", "raw__mitxonline__openedx__mysql__assessment_assessmentfeedback") }}
    ),
    afa as (
        select *
        from
            {{
                source(
                    ...
                    "raw__mitxonline__openedx__mysql__assessment_assessmentfeedback_assessments",
                )
            }}
    ),
    a as (
...
select distinct afo.assessmentfeedbackoption_id, afo.assessmentfeedback_id, afo.id
from afo
