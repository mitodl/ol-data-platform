with
    discussion_component_topics as (
        select
            *,
        ...
        where block_category = 'discussion' and is_latest = true
    ),
    course_level_discussion_topics as (
        ...
        cross join
            unnest(cast(json_extract(course.block_metadata, '$.discussion_topics') as map(varchar, json))) as t(
                key, topic
            )  -- noqa
        where course.block_category = 'course' and course.is_latest = true
