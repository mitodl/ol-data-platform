...
with
    ...
    mitxonline_openedx_users as (
        select openedx_user_id, user_username, user_email from {{ ref("stg__mitxonline__openedx__mysql__auth_user") }}
    ),
    ...
    -- MITx Pro Users
    mitxpro_users as (
        select user_id, user_username, user_email, user_full_name, user_is_active, user_joined_on
        from {{ ref("stg__mitxpro__app__postgres__users_user") }}
    ),
    mitxpro_legaladdress as (
        select user_id, user_address_country from {{ ref("stg__mitxpro__app__postgres__users_legaladdress") }}
    ),
    mitxpro_profile as (
        select
            user_id, user_highest_education, user_gender, user_birth_year, user_company, user_job_title, user_industry
        from {{ ref("stg__mitxpro__app__postgres__users_profile") }}
    ),
    mitxpro_openedx_users as (
        select openedx_user_id, user_username, user_email from {{ ref("stg__mitxpro__openedx__mysql__auth_user") }}
    ),
        ...
        from
            (
                select
                    ...
                    user_industry,
                    row_number() over (
                        partition by user_email order by user_gdpr_consent_date desc, courserun_start_on desc
                    ) as row_num
    ...
    mitxresidential_openedx_users as (
        select user_username, user_email, user_full_name, user_is_active, user_id, user_joined_on
        from {{ ref("stg__mitxresidential__openedx__auth_user") }}
    ),
    mitxresidential_profile as (
        select user_address_country, user_highest_education, user_gender, user_birth_year, user_id
    ...
    mitx_users_view as (
        ...
        from mitx_users
        left join
            mitxonline_app_openedxuser_mapping
    ...
    learn_user as (
        ...
        from
            (
                select *, row_number() over (partition by user_email order by user_created_on desc) as row_num
                from {{ ref("stg__mitlearn__app__postgres__users_user") }}
            )
        where row_num = 1
    ),
    learn_profile as (select * from {{ ref("stg__mitlearn__app__postgres__profiles_profile") }}),
    learn_user_topic_interests as (
        select profile_topic_interests.profile_id, array_agg(topic.learningresourcetopic_name) as topic_interests
        from {{ ref("stg__mitlearn__app__postgres__profiles_profile_topic_interests") }} as profile_topic_interests
        join
            {{ ref("stg__mitlearn__app__postgres__learning_resources_learningresourcetopic") }} as topic
    ...
    combined_users as (
        ...
        left join
            mitxpro_openedx_users as openedx_users_username
            on mitxpro_user_view.user_username = openedx_users_username.user_username
        left join
            mitxpro_openedx_users as openedx_users_email
    ...
    base_info as (select * from ranked_users where row_num = 1),
    agg_view as (
        select
            user_pk,
