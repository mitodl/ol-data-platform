with
    ...
    video as (
        ...
        select video_edx_uuid, video_duration, video_created_on
        ...
        select video_block_id as video_edx_uuid, video_duration, null as video_created_on
    ...
    video_duration as (
        select
            video_edx_uuid,
            video_duration,
            row_number() over (partition by video_edx_uuid order by video_created_on desc) as row_num
        from video
    ),
    combined as (
        select
        ...
        left join
            video_duration
            on video_content.video_edx_uuid = video_duration.video_edx_uuid
