...
, start_and_end_times as (
    select 
        ...
        , video_id
        , min(case when useractivity_event_name = 'play_video' then currenttime end) as start_time
        , max(case when useractivity_event_name <> 'play_video' then currenttime end) as end_time
