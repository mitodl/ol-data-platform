with
    ...
    tfact_video_events as (select * from {{ ref("tfact_video_events") }}),
    d_video as (select * from {{ ref("dim_video") }}),
    start_and_end_times as (
        select
            ...
            min(
                case when event_type = 'play_video' then (case when video_position = 'null' then '0' end) end
            ) as start_time
        from tfact_video_events
        where event_type in ('play_video', 'seek_video', 'pause_video', 'stop_video', 'complete_video')
        group by openedx_user_id, courserun_readable_id, video_block_fk
    )
...
inner join
    d_video
    on tfact_video_events.video_block_fk
...
inner join course_content as c on d_video.content_block_fk = c.content_block_pk and c.is_latest = true
left join course_content as d on c.parent_block_id = d.block_id and d.is_latest = true
left join course_content as f on d.parent_block_id = f.block_id and f.is_latest = true
left join course_content as g on f.parent_block_id = g.block_id and g.is_latest = true
left join
    start_and_end_times
    on tfact_video_events.video_block_fk = start_and_end_times.video_block_fk
