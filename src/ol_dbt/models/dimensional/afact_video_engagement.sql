...
, mitxonline_video_events as (
    select
        ...
        , lag(json_query(a.useractivity_event_object, 'lax $.currentTime'), 1)
            over (
                partition by
                    a.user_username, a.courserun_readable_id
...
, sum_mitxonline_video_events as (
    select
        ...
        , sum(case
            when event_type = 'play_video'
                then
                    cast(nextcurrenttime as decimal(30, 10))
                    - cast(currenttime as decimal(30, 10))
            else 0
        end) as time_played
