with
    program_enrollments as (select * from {{ ref("marts__combined_program_enrollment_detail") }}),
    course_enrollments as (select * from {{ ref("marts__combined_course_enrollment_detail") }}),
    program_courses as (select * from {{ ref("marts__combined_coursesinprogram") }}),
    aggregated_course_enrollments as (
        select
            program_courses.program_name,
            count(
                distinct course_enrollments.courserun_readable_id || course_enrollments.user_email
            ...
            count(
                distinct case
                    when course_enrollments.courseruncertificate_is_earned = true then course_enrollments.user_email
                end
    ...
    aggregated_program_certificates as (
        select program_name, count(distinct user_email) as program_certificates
...
left join
    aggregated_program_certificates
