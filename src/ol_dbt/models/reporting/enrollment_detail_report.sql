with
    enrollments as (select * from {{ ref("marts__combined_course_enrollment_detail") }}),
    ...
    coursesinprogram as (select * from {{ ref("marts__combined_coursesinprogram") }}),
    programs as (
        select distinct
    ...
    course_passed_counts as (
        select user_email, count(distinct course_title) as num_of_course_passed
        from enrollments
...
from enrollments
left join orders on enrollments.order_id = orders.order_id and enrollments.platform = orders.platform
