with
    video_pre_query as (
        ...
        where event_type in ('play_video', 'seek_video', 'pause_video', 'stop_video', 'complete_video')
        group by platform, openedx_user_id, courserun_readable_id, video_block_fk
    ),
    discuss_table as (
        select
            a.platform,
        ...
        left join
            ol_warehouse_production_dimensional.dim_user as u
            on a.platform = 'mitxonline'
            and a.openedx_user_id = u.mitxonline_openedx_user_id
        left join
            ol_warehouse_production_dimensional.dim_user as ou
            on a.platform = 'edxorg'
            and a.openedx_user_id = ou.edxorg_openedx_user_id
        left join
            ol_warehouse_production_intermediate.int__combined__course_runs as h2
            on a.courserun_readable_id = h2.courserun_readable_id
        left join
            ol_warehouse_production_dimensional.dim_course_content as c
            on a.sequential_block_fk = c.block_id
            and c.is_latest = true
        left join
            ol_warehouse_production_dimensional.dim_course_content as d
            on a.chapter_block_fk = d.block_id
    ...
    page_views_table as (
        ...
        left join
            ol_warehouse_production_dimensional.dim_course_content as b
            on a.block_fk = b.block_id
            and b.is_latest = true
        left join
            ol_warehouse_production_dimensional.dim_course_content as c
            on b.parent_block_id = c.block_id
            and c.is_latest = true
        left join
            ol_warehouse_production_dimensional.dim_course_content as d
            on c.parent_block_id = d.block_id
            and d.is_latest = true
        left join
            ol_warehouse_production_dimensional.dim_user as u
            on a.platform = 'mitxonline'
    ...
    video_views_table as (
        ...
        inner join
            ol_warehouse_production_dimensional.dim_video as c
            on a.courserun_readable_id = c.courserun_readable_id
            and a.video_block_fk = substring(c.video_block_pk, regexp_position(c.video_block_pk, 'block@') + 6)
        left join
            ol_warehouse_production_dimensional.dim_user as b
            on a.platform = 'mitxonline'
            and a.openedx_user_id = b.mitxonline_openedx_user_id
        left join
            ol_warehouse_production_dimensional.dim_user as ob
            on a.platform = 'edxorg'
        ...
        left join
            ...
            on a.courserun_readable_id = h2.courserun_readable_id
        left join
            ol_warehouse_production_dimensional.dim_course_content as v
            on c.content_block_fk = v.content_block_pk
            and a.courserun_readable_id = v.courserun_readable_id
        left join
            ol_warehouse_production_dimensional.dim_course_content as cc_subsection
            on v.parent_block_id = cc_subsection.block_id
            and a.courserun_readable_id = cc_subsection.courserun_readable_id
            and cc_subsection.is_latest = true
        left join
            ol_warehouse_production_dimensional.dim_course_content as cc_section
            on cc_subsection.parent_block_id = cc_section.block_id
    ...
    pre_problems_table as (
        select c.sequential_block_id, count(p.problem_block_pk) as problem_numb
        from ol_warehouse_production_dimensional.dim_problem as p
        inner join
            ol_warehouse_production_dimensional.dim_course_content as c on p.content_block_fk = c.content_block_pk
    ...
    problems_events as (
        select
            ...
            courserun_readable_id,
            openedx_user_id,
            avg(cast(grade as decimal(30, 10)) / nullif(cast(max_grade as decimal(30, 10)), 0)) as avg_percent_grade
        from ol_warehouse_production_dimensional.tfact_problem_events
        group by problem_block_fk, courserun_readable_id, openedx_user_id
    ),
    problems_table as (
        select
            a.platform,
            ...
            count(
                distinct case when cast(a.num_of_attempts as int) > 0 then a.problem_block_fk end
            ) as problems_attempted,
            max(c.problem_numb) as number_of_problems,
            cast(
                count(distinct case when cast(a.num_of_attempts as int) > 0 then a.problem_block_fk end) as decimal(
                    30, 10
                )
            )
            / cast(max(c.problem_numb) as decimal(30, 10)) as percetage_problems_attempted
        from ol_warehouse_production_dimensional.afact_problem_engagement as a
        inner join pre_problems_table as c on a.sequential_block_fk = c.sequential_block_id
        left join
            ol_warehouse_production_dimensional.dim_user as u
            on a.platform = 'mitxonline'
            and a.openedx_user_id = u.mitxonline_openedx_user_id
        ...
        inner join
            ol_warehouse_production_dimensional.dim_course_content as d
            on a.sequential_block_fk = d.block_id
            and d.is_latest = true
        inner join
            ol_warehouse_production_dimensional.dim_course_content as sec
            on a.chapter_block_fk = sec.block_id
            and sec.is_latest = true
        inner join
            problems_events as g
            on a.problem_block_fk = g.problem_block_fk
    ...
    page_and_video as (
        select
            ...
            coalesce(
                video_views_table.courserun_readable_id, page_views_table.courserun_readable_id
        ...
        full outer join
            video_views_table
            on page_views_table.email = video_views_table.email
    ...
    page_video_problems as (
        select
            ...
            coalesce(
                page_and_video.courserun_readable_id, problems_table.courserun_readable_id
            ) as courserun_readable_id,
        ...
        full outer join
            problems_table
            on page_and_video.email = problems_table.email
...
full outer join
    discuss_table
    on page_video_problems.email = discuss_table.email
