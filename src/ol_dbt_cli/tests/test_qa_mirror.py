...
class TestRenderMirror:
    def test_copies_masks_and_drops(self) -> None:
        ...
        assert "`user id`,\n" in statement.sql
        assert "sha2(`email`, 256) AS `email`" in statement.sql
    ...
    def test_hash_refuses_a_non_string_column(self) -> None:
        # sha2 would turn an int into a hex string under staging models that cast it.
        with pytest.raises(MirrorDeclarationError, match="needs a string column"):
            render_mirror(_table({"birth_year": "hash"}), PRODUCTION_TYPES)
