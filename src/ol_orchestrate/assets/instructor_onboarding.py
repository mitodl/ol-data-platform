...
dbt model and pushes them to a private GitHub repository for instructor access management.
"""
...
def generate_instructor_onboarding_user_list(
    ...
    user_data = (
        int__combined__user_course_roles
        .select(["user_email"])
    ...
    user_data = user_data.with_columns([
        pl.lit("ol-data-analyst").alias("role"),
        pl.lit(1).alias("sent_invite")
    ])
    ...
    context.log.info(f"Generated CSV content with {len(user_data)} unique users")
...
def update_access_forge_repository(
    ...
    try:
        ...
        context.log.info(f"Created branch {new_branch}")
        ...
        try:
            ...
            context.log.info(f"Updated file {file_path} in branch {new_branch}")
            action = "updated"
        except Exception:
            ...
            context.log.info(f"Created file {file_path} in branch {new_branch}")
            action = "created"
        
        # Create pull request
        pr = repo.create_pull(
            title=f"Update user list - {timestamp}",
            body="Automated update of user list from ol-data-platform Dagster pipeline.\n\n"
        ...
        context.log.info(f"Created PR #{pr.number}: {pr.html_url}")
        ...
        context.log.error(f"Failed to create PR in GitHub repository: {str(e)}")
        raise
