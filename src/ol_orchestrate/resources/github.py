...
class GithubApiClientFactory(ConfigurableResource):
    ...
    vault: Vault = Field(
        description="Vault resource for retrieving GitHub API token"
    )
    ...
    vault_secret_path: str = Field(
        default="pipelines/github/api",
        description="Path to GitHub API token in Vault"
    )
    
    _client: Optional[Github] = PrivateAttr(default=None)
