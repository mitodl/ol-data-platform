  ...
  create or replace view
    ol_data_lake_production.ol_warehouse_production_intermediate.int__combined__courserun_enrollments
  security definer
  as
...
with mitxonline_enrollments as (
    select * from ol_data_lake_production.ol_warehouse_production_intermediate.int__mitxonline__courserunenrollments
)

, mitxpro_enrollments as (
    select * from ol_data_lake_production.ol_warehouse_production_intermediate.int__mitxpro__courserunenrollments
)

, bootcamps_enrollments as (
    select * from ol_data_lake_production.ol_warehouse_production_intermediate.int__bootcamps__courserunenrollments
)

, edxorg_enrollments as (
    select * from ol_data_lake_production.ol_warehouse_production_intermediate.int__edxorg__mitx_courserun_enrollments
)
