"""Postgres schedule storage with connection pooling."""
...
class PooledPostgresScheduleStorage(PostgresScheduleStorage):
    ...
    This is a drop-in replacement for PostgresScheduleStorage that uses QueuePool
    ...
    def __init__(  # noqa: PLR0913
        ...
        """Initialize PooledPostgresScheduleStorage.
        ...
        if self.should_autocreate_tables:
            ...
            missing_main_table = (
                "schedules" not in table_names and "jobs" not in table_names
            )
            if missing_main_table:
                retry_pg_creation_fn(self._init_db)

        super(PostgresScheduleStorage, self).__init__()
