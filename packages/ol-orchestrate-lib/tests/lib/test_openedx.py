...
'''
    ...
    static_dir.mkdir()
    (static_dir / "subtitle.srt").write_text("1\n00:00:00,000 --> 00:00:05,000\nSample subtitle")
...
def test_process_course_xml_blocks_basic(sample_course_archive):
    ...
    for block in blocks:
        ...
        assert block["source_system"] == source_system, "Block should have correct source_system"
...
def test_process_course_xml_blocks_block_types(sample_course_archive):
    ...
    expected_types = {"chapter", "sequential", "vertical", "video", "problem", "html", "course"}
    assert expected_types.issubset(block_types), f"Should extract all expected block types. Found: {block_types}"
...
def test_process_course_xml_blocks_video_metadata(sample_course_archive):
    ...
    assert video_block["edx_video_id"] == "test-video-123", "Should extract correct video ID"
...
def test_process_course_xml_blocks_xml_attributes(sample_course_archive):
    ...
    for block in blocks:
        ...
        assert isinstance(block["xml_attributes"], dict), "xml_attributes should be a dict"
        
        # Blocks with display_name should have it in attributes
        if "block_display_name" in block and block["block_display_name"]:
            assert "display_name" in block["xml_attributes"], "display_name should be in xml_attributes"
...
def test_process_course_xml_blocks_source_system_tracking():
    ...
    for source_sys in ["prod", "edge", "mitxonline", "xpro"]:
        ...
        for block in blocks:
            assert block["source_system"] == source_sys, f"Block should have source_system={source_sys}"
