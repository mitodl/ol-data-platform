...
def _conversation_row(
    cluster_key: str, text: str | None, tag: str | None
) -> dict[str, str | None]:
    return {
...
def test_build_cluster_prompt_inputs_groups_by_cluster_key() -> None:
    df = pl.DataFrame(
        [
            _conversation_row("key-a", "turn a", "billing"),
            _conversation_row("key-b", "turn b", "login"),
        ]
    )
...
def test_build_cluster_prompt_inputs_picks_most_common_tag() -> None:
    df = pl.DataFrame(
        [
            _conversation_row("key-a", "a", "billing"),
            _conversation_row("key-a", "b", "billing"),
            _conversation_row("key-a", "c", "login"),
        ]
    )

    result = categorize.build_cluster_prompt_inputs(df, dominant_tag_count=1)

    assert result["key-a"]["dominant_tags"] == ["billing"]
    assert result["key-a"]["total_conversations"] == 3


def test_build_cluster_prompt_inputs_caps_sample_size() -> None:
    df = pl.DataFrame(
        [_conversation_row("key-a", f"turn {i}", None) for i in range(20)]
    )

    result = categorize.build_cluster_prompt_inputs(df, sample_size=5)

    assert len(result["key-a"]["samples"]) == 5


def test_build_cluster_prompt_inputs_drops_null_text_from_samples() -> None:
    df = pl.DataFrame(
        [
            _conversation_row("key-a", None, "billing"),
            _conversation_row("key-a", "real text", "billing"),
        ]
    )

    result = categorize.build_cluster_prompt_inputs(df)

    assert result["key-a"]["samples"] == ["real text"]
...
def test_propose_categories_builds_one_row_per_cluster() -> None:
    cluster_prompt_inputs = {
        ...
        "key-b": {"samples": ["c"], "dominant_tags": [], "total_conversations": 1},
    }
...
def test_propose_categories_skips_a_cluster_whose_call_fails() -> None:
    ...
    cluster_prompt_inputs = {
        "key-a": {"samples": ["a"], "dominant_tags": [], "total_conversations": 1},
    }
