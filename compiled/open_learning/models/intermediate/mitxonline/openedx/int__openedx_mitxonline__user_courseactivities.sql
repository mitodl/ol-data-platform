...
with person_courses as (
    ...
    where courserun_platform = 'MITx Online'
)

--- course info should be pulled from MITxOnline application database
, runs as (
    select * from dev.main_intermediate.int__mitxonline__course_runs
)

, open_edx_users as (
    select * from dev.main_intermediate.int__openedx_mitxonline__users
)
