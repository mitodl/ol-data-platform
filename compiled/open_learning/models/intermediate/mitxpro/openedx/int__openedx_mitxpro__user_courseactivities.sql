...
with person_courses as (
    ...
    where courserun_platform = 'xPro'
)

--- course info should be pulled from MIT xPro application database
, runs as (
    select * from dev.main_intermediate.int__mitxpro__course_runs
)

, open_edx_users as (
    select * from dev.main_intermediate.int__openedx_mitxpro__users
)
...
, user_activities as (
    select
        ...
        , open_edx_users.mitxpro_user_id
